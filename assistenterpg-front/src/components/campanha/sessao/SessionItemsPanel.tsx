  ...
  async function handleSalvarEdicao() {
    ...
    try {
      const payload = {
        ...
        descricaoRevelada: descricaoReveladaEdicao,
      };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded border border-app-border bg-app-surface p-3 space-y-2">
        <Input
          label={salvarComoTemplate ? 'Nome do template' : 'Nome do item'}
          value={nome}
          ...
          <div className="flex flex-wrap gap-3 text-xs text-app-muted">
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(event) => setSalvarComoTemplate(event.target.checked)}
              />
              Salvar como template
            </label>
          </div>
        ) : null}
        <Button size="sm" onClick={() => void handleCriarItem()} disabled={salvando}>
          {salvando
            ? 'Salvando...'
            : salvarComoTemplate && ehMestre
              ? 'Criar template'
              : 'Criar item'}
        </Button>
      </div>
          ...
          {itensFiltrados.map((item) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                {ehMestre || item.permissoes?.podeEditar ? (
