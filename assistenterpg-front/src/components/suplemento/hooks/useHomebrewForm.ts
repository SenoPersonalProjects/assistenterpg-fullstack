...
function sanitizeHomebrewDados(value: unknown): unknown {
  ...
  if (Array.isArray(value)) {
    return value
      .map((item) => sanitizeHomebrewDados(item))
  ...
  if (value && typeof value === "object") {
    const entries = Object.entries(value as Record<string, unknown>)
      .map(
        ([key, entryValue]) =>
          [key, sanitizeHomebrewDados(entryValue)] as const,
      )
...
export function useHomebrewForm({ initialValues }: Params = {}) {
  ...
  // ==================== DADOS ESPECÍFICOS ====================
  const [dados, setDados] = useState<HomebrewFormDados>(
    toHomebrewFormDados(initialValues?.dados),
  );
  ...
  const updateDados = useCallback(
    (partialDados: Partial<HomebrewFormDados>) => {
  ...
  const validar = useCallback((): { valido: boolean; erros: string[] } => {
    ...
    switch (tipo) {
      ...
      case TipoHomebrewConteudo.ORIGEM:
        if (
          !dados.pericias ||
          !Array.isArray(dados.pericias) ||
          dados.pericias.length === 0
        ) {
      ...
      case TipoHomebrewConteudo.EQUIPAMENTO:
        ...
        if (tipoEquipamento === TipoEquipamento.ARMA) {
          if (!dados.proficienciaArma)
            erros.push("Proficiência de arma é obrigatória");
          ...
          if (dados.criticoValor == null)
            erros.push("Crítico (valor) é obrigatório");
          if (dados.criticoMultiplicador == null)
            erros.push("Crítico (multiplicador) é obrigatório");
          if (dados.agil == null) erros.push("Defina se a arma é ágil");
        }

        if (tipoEquipamento === TipoEquipamento.PROTECAO) {
          if (!dados.proficienciaProtecao)
            erros.push("Proficiência de proteção é obrigatória");
          if (!dados.tipoProtecao) erros.push("Tipo de proteção é obrigatório");
          if (dados.bonusDefesa == null)
            erros.push("Bônus de defesa é obrigatório");
          if (dados.penalidadeCarga == null)
            erros.push("Penalidade de carga é obrigatória");
        }

        if (tipoEquipamento === TipoEquipamento.ACESSORIO) {
          if (!dados.tipoAcessorio)
            erros.push("Tipo de acessório é obrigatório");
        }

        if (tipoEquipamento === TipoEquipamento.MUNICAO) {
          if (!dados.duracaoCenas) erros.push("Duração (cenas) é obrigatória");
          if (dados.recuperavel == null)
            erros.push("Defina se a munição é recuperável");
        }

        if (tipoEquipamento === TipoEquipamento.EXPLOSIVO) {
          if (!dados.tipoExplosivo)
            erros.push("Tipo de explosivo é obrigatório");
          if (!dados.efeito?.trim())
            erros.push("Efeito do explosivo é obrigatório");
        }
        ...
        if (tipoEquipamento === TipoEquipamento.FERRAMENTA_AMALDICOADA) {
          ...
          const temArma =
            dados.armaAmaldicoada && dados.armaAmaldicoada.tipoBase;
          const temProtecao =
            dados.protecaoAmaldicoada && dados.protecaoAmaldicoada.tipoBase;
          const temArtefato =
            dados.artefatoAmaldicoado && dados.artefatoAmaldicoado.tipoBase;
          ...
          if (
            dados.tipoAmaldicoado === TipoAmaldicoado.ARTEFATO &&
            !temArtefato
          ) {
            erros.push("Selecione os dados de artefato amaldicoado");
          }
        }
        ...
        if (Array.isArray(dados.habilidades)) {
          dados.habilidades.forEach((hab, idx: number) => {
            ...
            if (!hab.descricao?.trim())
              erros.push(`Habilidade #${idx + 1}: Descrição é obrigatória`);
            if (!hab.execucao)
              erros.push(`Habilidade #${idx + 1}: Execução é obrigatória`);
            if (hab.custoPE == null)
              erros.push(`Habilidade #${idx + 1}: Custo PE é obrigatório`);
            if (hab.custoEA == null)
              erros.push(`Habilidade #${idx + 1}: Custo EA é obrigatório`);
            if (!hab.efeito?.trim())
              erros.push(`Habilidade #${idx + 1}: Efeito é obrigatório`);
          });
        }
        break;
    }
  ...
  const buildPayload = useCallback((): CreateHomebrewDto => {
    ...
    const dadosSanitizados = sanitizeHomebrewDados(dados);
    ...
    return {
      ...
      versao,
      dados: dadosSanitizados as JsonValue,
    };
