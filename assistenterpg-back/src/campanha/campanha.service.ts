...
export class CampanhaService {
  ...
  async excluirCampanha(campanhaId: number, usuarioId: number) {
    ...
    await this.prisma.executarTransacao('campanha.excluir', async (tx) => {
      await bloquearCampanhaTx(tx, campanhaId);
      ...
      await this.personagensService.removerTecnicasInatasClonadas(
        personagensCampanha
          .map((personagem) => personagem.tecnicaInataPropriaId)
