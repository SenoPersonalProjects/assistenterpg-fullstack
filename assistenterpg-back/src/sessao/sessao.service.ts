...
import {
  ...
  bloquearMacroPersonagemCampanhaTx,
...
import {
  ...
  CriarRolagemMacroPersonagemSessaoDto,
...
type ProcessamentoEfeitosTurnoSessao = {
  ...
  campanhaId: number;
  sessaoId: number;
  contexto: ContextoEfeitosTurnoSessao;
};

type EfeitosTurnoPendentesResumo = {
  eventoId: number;
...
const TIPOS_EVENTO_AJUSTE_TURNO = [
  'TURNO_AVANCADO',
  'TURNO_RECUADO',
  'TURNO_PULADO',
...
export class SessaoService {
  ...
  async encerrarSessaoCampanha(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const efeitosPendentes = await this.obterEfeitosTurnoPendentesTx(
        tx,
        sessaoId,
      );
      if (efeitosPendentes) {
        throw new SessaoEfeitosTurnoPendentesException(
          campanhaId,
          sessaoId,
          efeitosPendentes.eventoId,
        );
      }
  ...
  async buscarDetalheSessao(
    ...
    const efeitosTurnoPendentes = await this.obterEfeitosTurnoPendentesTx(
      this.prisma,
      sessaoId,
    );
    const dominiosAtivos = await this.prisma.dominioSessao.findMany({
      ...
      include: {
        personagemSessao: {
          include: {
            personagemCampanha: { select: { nome: true, donoId: true } },
          },
        },
        npcSessao: {
          select: { nomeExibicao: true, controladorUsuarioId: true },
        },
    ...
    const dominios = dominiosAtivos.map((dominio) => {
      ...
      return {
        ...
        grauBarreira: dominio.grauBarreira,
        integridadeMax: dominio.integridadeMax,
        integridadeAtual: dominio.integridadeAtual,
        rupturas: dominio.rupturas,
        estadoBarreira:
          dominio.tipo === 'ABERTO'
            ? 'ABERTO'
            : dominio.integridadeMax !== null &&
                dominio.rupturas >= dominio.integridadeMax
              ? 'COLAPSADA'
              : dominio.rupturas >= 2
                ? 'BRECHA'
                : dominio.rupturas === 1
        ...
        participante: dominio.personagemSessao
          ? {
              tipo: 'PERSONAGEM',
              id: dominio.personagemSessaoId,
              nome: dominio.personagemSessao.personagemCampanha.nome,
            }
          : {
              tipo: 'NPC',
              id: dominio.npcSessaoId,
              nome: dominio.npcSessao?.nomeExibicao ?? 'NPC',
            },
        podeControlar,
        acertoGarantido:
        ...
        disputas: dominio.participacoesDisputa.map((participacao) => ({
          ...
          estado: participacao.disputa.estado,
          dominancia: participacao.dominancia,
          resolucoesConcluidas: participacao.disputa.resolucoesConcluidas,
          rodadaProximaResolucao: participacao.disputa.rodadaProximaResolucao,
        })),
      };
    });
  ...
  async criarRolagemSessao(
    ...
    if (
      ...
      return this.criarRolagemMacroPersonagemSessao(
        campanhaId,
        sessaoId,
        usuarioId,
        dto as CriarRolagemMacroPersonagemSessaoDto,
      );
    }
  ...
  private async criarRolagemMacroPersonagemSessao(
    campanhaId: number,
    sessaoId: number,
    usuarioId: number,
    dto: CriarRolagemMacroPersonagemSessaoDto,
  ) {
    const inicio = Date.now();
    const descricaoAcao =
      dto.tipo === 'ATAQUE_MACRO_PERSONAGEM'
        ? 'realizar ataque de macro personalizada'
        : dto.tipo === 'DANO_MACRO_PERSONAGEM'
          ? 'realizar dano de macro personalizada'
          : dto.tipo === 'CRITICO_MACRO_PERSONAGEM'
            ? 'realizar critico de macro personalizada'
            : 'realizar formula de macro personalizada';
    const { acesso } = await this.obterSessaoComAcesso(
      campanhaId,
      sessaoId,
      usuarioId,
    );
    const intencao = this.montarIntencaoRolagemMacro(dto);
    const existente = await this.buscarEventoRolagemIdempotente(
      this.prisma,
      sessaoId,
      usuarioId,
      ...
      resultadoTransacao = await executarComRetryConcorrencia(
        ...
        () =>
          this.prisma.$transaction(async (tx) => {
            ...
            await bloquearMacroPersonagemCampanhaTx(
              tx,
              campanhaId,
              personagemInicial.personagemCampanhaId,
              dto.macroId,
            ...
            const [macro, personagem] = await Promise.all([
              tx.personagemCampanhaMacro.findFirst({
                where: {
                  id: dto.macroId,
                  campanhaId,
                  personagemCampanhaId: personagemInicial.personagemCampanhaId,
                  ativo: true,
                },
              }),
            ...
            if (!macro) {
              throw new MacroPersonagemNaoEncontradaException(dto.macroId);
            }
            if (!personagem) {
              throw new PersonagemSessaoNaoEncontradoException(
                dto.personagemSessaoId,
            ...
            const tiposCompativeis: Record<
              CriarRolagemMacroPersonagemSessaoDto['tipo'],
              MacroPersonalizadaTipo
            > = {
              ATAQUE_MACRO_PERSONAGEM: 'ATAQUE_PERICIA',
              DANO_MACRO_PERSONAGEM: 'DANO_FORMULA',
              CRITICO_MACRO_PERSONAGEM: 'DANO_FORMULA',
              FORMULA_MACRO_PERSONAGEM: 'FORMULA_LIVRE',
            };
            if (macro.tipo !== tiposCompativeis[dto.tipo]) {
              throw new MacroPersonagemConfigInvalidaException(
                'O tipo da macro nao e compativel com a rolagem solicitada.',
              );
            }
            ...
            const visibilidade = dto.visibilidade ?? macro.visibilidadePadrao;
            if (visibilidade === 'SECRETA_MESTRE' && !acesso.ehMestre) {
            ...
            let formulaResolvida: string | null = null;
            let ajustesAutomaticos: Array<{
              condicao: string;
              dados: number;
              motivo: string;
            }> = [];
            let ajustesAplicados: Array<Record<string, unknown>> = [];
            let inspiracaoAutomatica: Record<string, unknown> | null = null;
            let dtAplicada: number | null = null;

            if (dto.tipo === 'ATAQUE_MACRO_PERSONAGEM') {
              const ataque = config as MacroAtaqueConfigV1;
              const ajusteFlat =
                ataque.ajusteFlatPadrao + (dto.ajusteFlatSessao ?? 0);
              const ajusteDados =
                ataque.ajusteDadosPadrao + (dto.ajusteDadosSessao ?? 0);
              if (
                Math.abs(ajusteFlat) > MACRO_PERSONAGEM_FLAT_ATAQUE_MAX ||
                Math.abs(ajusteDados) > MACRO_PERSONAGEM_DADOS_ATAQUE_MAX
              ) {
                throw new SessaoRolagemInvalidaException(
                  'Os ajustes combinados da macro excedem o limite permitido.',
                );
              }
              const pericia = await this.resolverPericiaAutoritativaTx(
                tx,
              ...
              ajustesAutomaticos = resolverAjustesAutomaticosAtaque({
              ...
              const dadosLogicos =
                pericia.valorAtributo + ajusteDados + ajusteAutomaticoDados;
              const quantidadeDados =
                this.calcularDadosPadraoPericia(dadosLogicos);
              ...
              const bonus = pericia.bonusTotal + escalada.bonus + ajusteFlat;
              const payload = rolarDadosServidor({
                ...
                label:
                  `${personagem.personagemCampanha.nome} · ${macro.nome}`.slice(
                    0,
                    LIMITES_DICE_SESSAO.label,
                  ),
              });
              dtAplicada = dto.contexto?.dt ?? ataque.dtPadrao ?? null;
              const resultado = calcularResultadoDiceServidor(payload);
              inspiracaoAutomatica =
                await this.resolverInspiracaoAutoritativaTx(
                  tx,
                  sessaoId,
                  personagem.personagemCampanhaId,
                  dtAplicada ?? undefined,
                  payload,
                  resultado.total,
                );
              payloads = [payload];
              ajustesAplicados = [
                ...ajustesAutomaticos.map((ajuste) => ({
                  tipo: 'CONDICAO_AUTOMATICA',
                  condicao: ajuste.condicao,
                  valor: ajuste.dados,
                  descricao: ajuste.motivo,
                })),
                ...escalada.ajustes.map((ajuste) =>
                  this.extrairRegistro(ajuste),
                ),
                ...(ataque.ajusteFlatPadrao
                  ? [
                      {
                        tipo: 'AJUSTE_FLAT_PADRAO',
                        valor: ataque.ajusteFlatPadrao,
                      },
                    ]
                  : []),
                ...(ataque.ajusteDadosPadrao
                  ? [
                      {
                        tipo: 'AJUSTE_DADOS_PADRAO',
                        valor: ataque.ajusteDadosPadrao,
                      },
                    ]
                  : []),
                ...(dto.ajusteFlatSessao
                  ? [
                      {
                        tipo: 'AJUSTE_FLAT_SESSAO',
                        valor: dto.ajusteFlatSessao,
                      },
                    ]
                  : []),
                ...(dto.ajusteDadosSessao
                  ? [
                      {
                        tipo: 'AJUSTE_DADOS_SESSAO',
                        valor: dto.ajusteDadosSessao,
                      },
                    ]
                  : []),
              ];
              ...
              const dano = config as MacroDanoConfigV1;
              if (
                dto.tipo === 'CRITICO_MACRO_PERSONAGEM' &&
                dano.criticoMultiplicador === undefined
              ) {
                throw new SessaoRolagemInvalidaException(
                  'A macro nao possui multiplicador de critico.',
                );
              }
              const normalizada = normalizarFormulaDanoMacro(dano.formulaBase);
              const ajusteFlat =
                dano.ajusteFlatPadrao + (dto.ajusteFlatSessao ?? 0);
              const modificadorBase =
                normalizada.expressao.operador === '-'
                  ? -normalizada.expressao.modificador
                  : normalizada.expressao.modificador;
              if (
                Math.abs(modificadorBase + ajusteFlat) >
                MACRO_PERSONAGEM_MODIFICADOR_DANO_MAX
              ) {
                throw new SessaoRolagemInvalidaException(
                  'O modificador efetivo de dano excede o limite permitido.',
                );
              }
              formulaBase = normalizada.formula;
              let expressao = this.somarModificadorMacroArma(
                {
                  ...normalizada.expressao,
                  label:
                    `${macro.nome}${dano.tipoDano ? ` · ${dano.tipoDano}` : ''}`.slice(
                      0,
                      LIMITES_DICE_SESSAO.label,
                    ),
                },
              ...
              if (dto.tipo === 'CRITICO_MACRO_PERSONAGEM') {
                expressao = this.multiplicarDadosMacroArma(
                  expressao,
                  dano.criticoMultiplicador!,
                );
              }
              payloads = [rolarDadosServidor(expressao)];
              formulaResolvida = formatarExpressaoDiceServidor(expressao);
              ajustesAplicados = [
                ...(dano.ajusteFlatPadrao
                  ? [
                      {
                        tipo: 'AJUSTE_FLAT_PADRAO',
                        valor: dano.ajusteFlatPadrao,
                      },
                    ]
                  : []),
                ...(dto.ajusteFlatSessao
              ...
              const livre = config as MacroFormulaLivreConfigV1;
              const parse = parseDiceInputServidor(livre.formula);
              if (parse.erro || !parse.expressions) {
                throw new SessaoRolagemInvalidaException(
                  parse.erro ?? 'Formula da macro invalida.',
                );
              }
              formulaBase = livre.formula;
              payloads = parse.expressions.map((expressao) =>
                rolarDadosServidor({
                  ...expressao,
                  label: (expressao.label ?? macro.nome).slice(
                    0,
                    LIMITES_DICE_SESSAO.label,
                  ),
                }),
              );
              formulaResolvida = payloads
                .map((payload) => formatarExpressaoDiceServidor(payload))
                .join(' + ');
            }
              ...
              data: {
                ...
                dados: this.jsonParaPersistencia({
                  ...
                  dadosRolagem: {
                    ...
                    personagemCampanhaId: personagem.personagemCampanhaId,
                    macroId: macro.id,
                    macroRevisao: macro.revisao,
                    macroNome: macro.nome,
                    macroTipo: macro.tipo,
                    configVersao: macro.configVersao,
                    configSnapshot: config,
                    ajusteFlatSessao: dto.ajusteFlatSessao ?? 0,
                    ajusteDadosSessao:
                      dto.tipo === 'ATAQUE_MACRO_PERSONAGEM'
                        ? (dto.ajusteDadosSessao ?? 0)
                        : null,
                    ajustesAplicados,
                    condicoesAutomaticas: ajustesAutomaticos,
                    formulaBase,
                    formulaResolvida,
                    formulasResolvidas,
                    payloads,
                    resultado: {
                      total,
                      dt: dtAplicada,
                      sucesso: dtAplicada === null ? null : total >= dtAplicada,
                      falhaCritica: inspiracaoAutomatica !== null,
                    },
                  },
                  contextoRolagem: {
    ...
    if (resultadoTransacao.criadoAgora && duracaoMs > 1000) {
      this.logger.warn(
        JSON.stringify({
          evento: 'rolagem_macro_personagem_sessao_lenta',
          sessaoId,
          eventoId: resultadoTransacao.evento.id,
          duracaoMs,
        }),
    ...
    const criadoAgora = await this.executarMutacaoIdempotenteSessao(
      ...
      () =>
        this.prisma.$transaction(async (tx) => {
          ...
          const mecanicaDominio = this.extrairRegistro(
            mecanicaSessao.dominio as Prisma.JsonValue,
          );
          const ehExpansaoDominio = mecanicaSessao.tipo === 'EXPANSAO_DOMINIO';
          const investimentoIntegridade =
            ehExpansaoDominio && mecanicaDominio.tipo === 'FECHADO'
              ? Math.min(
                  Math.max(0, Math.trunc(dto.investimentoIntegridade ?? 0)),
                  Math.max(0, grausMapEfetivo.get('TECNICA_BARREIRA') ?? 0),
                )
              : 0;
          if (ehExpansaoDominio && mecanicaDominio.tipo === 'FECHADO') {
            custo = {
              ...custo,
              custoEA: custo.custoEA + investimentoIntegridade * 2,
              custoEAOriginal:
          ...
          if (ehExpansaoDominio) {
            const tipoDominio =
              mecanicaDominio.tipo === 'ABERTO' ? 'ABERTO' : 'FECHADO';
            const grauBarreira = Math.max(
              2,
              grausMapEfetivo.get('TECNICA_BARREIRA') ?? 2,
            );
            const integridade =
              tipoDominio === 'FECHADO'
                ? 4 + Math.max(0, grauBarreira - 2) + investimentoIntegridade
                : null;
            const dominio = await tx.dominioSessao.create({
              data: {
                ...
                personagemSessaoId,
                habilidadeTecnicaId: habilidade.id,
                sustentacaoHabilidadeId: sustentacaoCriadaId,
                nome: habilidade.nome,
                descricaoAcertoGarantido: habilidade.efeito,
                tipo: tipoDominio,
                grauBarreira,
                integridadeMax: integridade,
                integridadeAtual: integridade,
                investimentoIntegridade,
          ...
          if (mecanicaSessao.tipo === 'DEFESA_ANTI_DOMINIO') {
            const defesa = this.extrairRegistro(
              mecanicaSessao.defesa as Prisma.JsonValue,
            );
            const tipoDefesa =
              defesa.tipo === 'CESTA_OCA' ||
              defesa.tipo === 'DOMINIO_SIMPLES' ||
              defesa.tipo === 'AMPLIFICACAO'
                ? defesa.tipo
                : null;
            if (tipoDefesa) {
              const grauAntiBarreira = Math.max(
                0,
                grausMapEfetivo.get('TECNICA_ANTI_BARREIRA') ?? 0,
              );
              const integridade =
                tipoDefesa === 'AMPLIFICACAO'
                  ? null
                  : (tipoDefesa === 'CESTA_OCA' ? 2 : 3) +
                    Math.ceil(grauAntiBarreira / 2);
              await tx.defesaAntiDominioSessao.updateMany({
                ...
                data: {
                  ...
                  motivoEncerramento: 'Substituída por nova ativação.',
                },
              });
              await tx.defesaAntiDominioSessao.create({
                data: {
                  sessaoId,
                  cenaId: cenaAtual.id,
                  personagemSessaoId,
  ...
  async reprocessarEfeitosAutomaticosTurnoSessao(
    ...
    assertSessaoMutavel(
      ...
      campanhaId,
      sessaoId,
      'reprocessar efeitos automáticos do turno',
    );
    this.assertMestre(acesso, 'reprocessar efeitos automáticos do turno');

    const processamento = await this.obterProcessamentoEfeitosTurnoPorEvento(
      campanhaId,
      sessaoId,
      eventoId,
    );
    const concluido =
      await this.processarEfeitosAutomaticosTurnoSessao(processamento);
    if (!concluido) {
      throw new SessaoEfeitosTurnoFalharamException(
        campanhaId,
        sessaoId,
        eventoId,
      );
  ...
  private montarIntencaoRolagemMacro(
    dto: CriarRolagemMacroPersonagemSessaoDto,
  ): Record<string, unknown> {
    return {
      tipo: dto.tipo,
  ...
  private normalizarConfigMacroPersistida(
  ...
  private multiplicarDadosMacroArma(
    ...
    const totalDados = contarDadosExpressaoMacro(expressao) * multiplicador;
    ...
    return {
      ...expressao,
      quantidade: expressao.quantidade * multiplicador,
  ...
  private async resolverPericiaAutoritativaTx(
    ...
    atributoBaseForcado?: AtributoMacroPersonagem,
  ): Promise<{
    personagemCampanhaId: number;
    personagemNome: string;
    donoId: number;
  ...
  private async aplicarAjusteLadoIniciativaAlternadaTx(
    ...
    if (iniciativa.ladoAtualId !== args.ladoAtualIdEsperado) {
      throw new SessaoTurnoDesatualizadoException(
        args.campanhaId,
        args.sessao.id,
      );
    }
    ...
    const contextoEfeitos = criarContextoEfeitosTurno({
      ...
      participantesTurnoNovos:
        args.acao === 'VOLTAR' ? [] : participantesTurnoNovos,
    ...
    const evento = await tx.eventoSessao.create({
      data: {
        ...
        dados: this.jsonParaPersistencia({
          ...
          efeitosAutomaticos: contextoEfeitos,
        }),
      },
      select: {
        id: true,
  ...
  async criarDominioNpcSessao(
    ...
    sessaoId: number,
    usuarioId: number,
    dto: CriarDominioNpcSessaoDto,
  ) {
    const { acesso, sessao } = await this.obterSessaoMutavelComAcesso(
      campanhaId,
      sessaoId,
      usuarioId,
      'abrir Dominio de NPC',
    );
    ...
    const npc = await this.prisma.npcAmeacaSessao.findFirst({
      ...
      select: { id: true, eaAtual: true, peAtual: true },
    });
    if (!npc || !cena)
      throw new BusinessException(
        'NPC nao esta na cena atual.',
        'DOMINIO_PARTICIPANTE_INVALIDO',
      );
    const investimento =
      dto.tipo === 'FECHADO'
        ? Math.min(dto.investimentoIntegridade ?? 0, dto.grauBarreira)
        : 0;
    const custoEA = (dto.custoEA ?? 0) + investimento * 2;
    if ((npc.eaAtual ?? 0) < custoEA || (npc.peAtual ?? 0) < (dto.custoPE ?? 0))
      throw new BusinessException(
        'Recursos insuficientes para abrir o Dominio.',
        'DOMINIO_RECURSOS_INSUFICIENTES',
      );
    const dominio = await this.prisma.$transaction(async (tx) => {
      if (custoEA || dto.custoPE)
        await tx.npcAmeacaSessao.update({
          where: { id: npc.id },
          data: {
            eaAtual: { decrement: custoEA },
            peAtual: { decrement: dto.custoPE ?? 0 },
          },
        });
      const criado = await tx.dominioSessao.create({
        data: {
          sessaoId,
          cenaId: cena.id,
          npcSessaoId: npc.id,
          nome: dto.nome,
          descricaoAcertoGarantido: dto.descricaoAcertoGarantido,
          tipo: dto.tipo,
          grauBarreira: dto.grauBarreira,
          investimentoIntegridade: investimento,
          rodadaAbertura: sessao.rodadaAtual,
          integridadeMax:
            dto.tipo === 'FECHADO'
              ? 4 + Math.max(0, dto.grauBarreira - 2) + investimento
              : null,
          integridadeAtual:
            dto.tipo === 'FECHADO'
              ? 4 + Math.max(0, dto.grauBarreira - 2) + investimento
  ...
  async executarAcaoDominioSessao(
    ...
    const dominio = await this.prisma.dominioSessao.findFirst({
      where: { id: dominioId, sessaoId },
      include: {
        personagemSessao: {
          include: { personagemCampanha: { select: { donoId: true } } },
        },
        npcSessao: { select: { controladorUsuarioId: true } },
      },
    });
    if (!dominio)
      throw new BusinessException(
    ...
    if (!podeControlar)
      throw new CampanhaAcessoNegadoException(campanhaId, usuarioId);
    if (
      ['FORMAR', 'INTERROMPER', 'REGISTRAR_RUPTURA'].includes(dto.acao) &&
      !acesso.ehMestre
    )
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (dto.acao === 'FORMAR') {
        if (atual.estado !== 'ABRINDO')
          throw new BusinessException(
            'Somente um Dominio em abertura pode ser formado.',
        ...
        if (atual.tipo !== 'FECHADO' || atual.estado !== 'ATIVO')
          throw new BusinessException(
            'Apenas Dominio fechado ativo pode reforcar a barreira.',
        ...
        if (atual.tipo !== 'FECHADO' || atual.integridadeMax === null)
          throw new BusinessException(
            'Dominio aberto nao possui integridade para romper.',
            'DOMINIO_SEM_INTEGRIDADE',
          );
        const dt = 15 + 3 * atual.grauBarreira;
        if ((dto.resultadoAtaque ?? 0) < dt)
          throw new BusinessException(
            'O resultado confirmado nao superou a DT estrutural.',
            'DOMINIO_DT_ESTRUTURAL_NAO_SUPERADA',
            { dt },
          );
        let ganho =
          dto.potenciaRuptura === 'EXCEPCIONAL'
            ? 3
            : dto.potenciaRuptura === 'POTENCIALIZADO'
              ? 2
              : 1;
        if (dto.golpeConcentrado) ganho = Math.min(3, ganho + 1);
        const rupturas = atual.rupturas + ganho;
        if (rupturas >= atual.integridadeMax)
          await this.encerrarDominioTx(
            tx,
            atual,
            usuarioId,
        ...
        if (!participante)
          throw new BusinessException(
            'Este Dominio nao participa de uma disputa ativa.',
            'DOMINIO_DISPUTA_AUSENTE',
          );
        if (dto.acao === 'REFINAR')
  ...
  async criarDisputaDominioSessao(
    ...
    sessaoId: number,
    usuarioId: number,
    dto: CriarDisputaDominioSessaoDto,
  ) {
    const { acesso, sessao } = await this.obterSessaoMutavelComAcesso(
      campanhaId,
      sessaoId,
      usuarioId,
      'criar disputa de Dominios',
    );
    this.assertMestre(acesso, 'criar disputa de Dominios');
    if (new Set(dto.dominioIds).size < 2)
      throw new BusinessException(
        'Uma disputa exige ao menos dois Dominios distintos.',
        'DOMINIO_DISPUTA_PARTICIPANTES_INSUFICIENTES',
      );
    const cena = await this.obterCenaAtualSessaoTx(this.prisma, sessaoId);
    const dominios = await this.prisma.dominioSessao.findMany({
      where: {
        id: { in: dto.dominioIds },
        sessaoId,
        cenaId: cena.id,
        estado: 'ATIVO',
      },
    ...
    if (dominios.length !== dto.dominioIds.length)
      throw new BusinessException(
    ...
    await this.prisma.$transaction(async (tx) => {
      const disputa = await tx.disputaDominioSessao.create({
        data: {
          sessaoId,
          cenaId: cena.id,
          rodadaProximaResolucao: sessao.rodadaAtual + 1,
          criadoPorUsuarioId: usuarioId,
        },
      });
      await tx.disputaDominioParticipante.createMany({
        data: dominios.map((dominio) => ({
          disputaDominioSessaoId: disputa.id,
          dominioSessaoId: dominio.id,
          primeiraResolucaoPendente: true,
        })),
      });
      await this.criarAlvosDisputaTx(
        tx,
        disputa.id,
        sessaoId,
        cena.id,
  ...
  async resolverDisputaDominioSessao(
    ...
    const { acesso, sessao } = await this.obterSessaoMutavelComAcesso(
      ...
      sessaoId,
      usuarioId,
      'resolver disputa de Dominios',
    );
    this.assertMestre(acesso, 'resolver disputa de Dominios');
    await this.prisma.$transaction(async (tx) => {
      const disputa = await tx.disputaDominioSessao.findFirst({
        where: { id: disputaId, sessaoId, estado: 'ATIVA' },
        include: { participantes: { include: { dominio: true } } },
      });
      if (!disputa)
        throw new BusinessException(
      ...
      if (
        resultados.length < 2 ||
        resultados[0].valor === resultados[1].valor
      ) {
        if (disputa.resolucoesConcluidas >= 2) {
        ...
        if (dominancia >= 3 || disputa.resolucoesConcluidas >= 2)
          await tx.disputaDominioSessao.update({
            ...
            data: {
              estado: 'ENCERRADA',
              encerradaEm: new Date(),
              motivoEncerramento: `VENCEDOR:${vencedor.dominioSessaoId}`,
            },
          });
        else
          await tx.disputaDominioSessao.update({
            where: { id: disputa.id },
            data: {
              resolucoesConcluidas: { increment: 1 },
              rodadaProximaResolucao: { increment: 1 },
            },
          });
      }
      await tx.eventoSessao.create({
        data: {
          sessaoId,
          cenaId: disputa.cenaId,
          tipoEvento: 'DOMINIO_DISPUTA_RESOLVIDA',
          solicitanteUsuarioId: usuarioId,
          clientRequestId: dto.clientRequestId,
          dados: this.jsonParaPersistencia({
            disputaId,
            resolucao: disputa.resolucoesConcluidas + 1,
  ...
  async criarDefesaAntiDominioSessao(
    ...
    sessaoId: number,
    usuarioId: number,
    dto: CriarDefesaAntiDominioSessaoDto,
  ) {
    const { acesso } = await this.obterSessaoMutavelComAcesso(
      campanhaId,
      sessaoId,
  ...
  async dormirInterludioSessao(
    ...
    personagemSessaoId: number,
    usuarioId: number,
    dto: DormirInterludioSessaoDto,
  ) {
    const { acesso, sessao } = await this.obterSessaoMutavelComAcesso(
      campanhaId,
      sessaoId,
      usuarioId,
      'dormir no interludio',
    );
    if (sessao.cenaAtualTipo !== 'LIVRE')
      throw new BusinessException(
        'Dormir em interludio exige uma cena livre.',
    ...
    if (
      ...
      throw new CampanhaAcessoNegadoException(campanhaId, usuarioId);
    const codigos = ['ESGOTAMENTO_DOMINIO', 'SOBRECARGA_NEURAL'];
    const condicoes = await this.prisma.condicao.findMany({
      where: { codigo: { in: codigos } },
      select: { id: true },
    });
    await this.prisma.condicaoPersonagemSessao.updateMany({
      where: {
        sessaoId,
        personagemSessaoId,
        ativo: true,
        condicaoId: { in: condicoes.map((c) => c.id) },
      },
      data: {
        ativo: false,
        removidaEm: new Date(),
        motivoRemocao: 'DORMIR_INTERLUDIO',
      },
    });
    await this.prisma.eventoSessao.create({
      data: {
        sessaoId,
        tipoEvento: 'DOMINIO_INTERLUDIO_DESCANSO',
        solicitanteUsuarioId: usuarioId,
        clientRequestId: dto.clientRequestId,
        dados: this.jsonParaPersistencia({ personagemSessaoId }),
      },
    });
    return this.buscarDetalheSessao(campanhaId, sessaoId, usuarioId);
  }

  private async criarAlvosDominioTx(
    tx: Prisma.TransactionClient,
    dominioSessaoId: number,
    sessaoId: number,
    cenaId: number,
    personagens: number[] | undefined,
    npcs: number[] | undefined,
    npcDonoId?: number,
  ) {
    const ps =
      personagens ??
      (
        await tx.personagemSessao.findMany({
          where: { sessaoId, cenaId },
          select: { id: true },
        })
      ).map((x) => x.id);
    const ns =
      npcs ??
      (
        await tx.npcAmeacaSessao.findMany({
          where: { sessaoId, cenaId },
          select: { id: true },
        })
      ).map((x) => x.id);
    ...
    await tx.dominioSessaoAlvo.createMany({
      data: [
        ...
        ...ns
          .filter((id) => id !== npcDonoId)
          .map((npcSessaoId) => ({ dominioSessaoId, npcSessaoId })),
      ],
    });
  }

  private async criarAlvosDisputaTx(
    tx: Prisma.TransactionClient,
    disputaDominioSessaoId: number,
    sessaoId: number,
    cenaId: number,
    personagens?: number[],
    npcs?: number[],
  ...
  private async cobrarEaDominioTx(
    tx: Prisma.TransactionClient,
    dominio: { personagemSessaoId: number | null; npcSessaoId: number | null },
    custo: number,
  ) {
    if (dominio.personagemSessaoId) {
      const p = await tx.personagemSessao.findUnique({
        where: { id: dominio.personagemSessaoId },
        include: { personagemCampanha: { select: { eaAtual: true } } },
      });
      if (!p || p.personagemCampanha.eaAtual < custo)
        throw new BusinessException(
          'EA insuficiente.',
          'DOMINIO_EA_INSUFICIENTE',
        );
      await tx.personagemCampanha.update({
        where: { id: p.personagemCampanhaId },
        data: { eaAtual: { decrement: custo } },
      });
    }
    if (dominio.npcSessaoId) {
      const n = await tx.npcAmeacaSessao.findUnique({
        where: { id: dominio.npcSessaoId },
        select: { eaAtual: true },
      });
      if (!n || (n.eaAtual ?? 0) < custo)
        throw new BusinessException(
          'EA insuficiente.',
          'DOMINIO_EA_INSUFICIENTE',
        );
  ...
  private async cobrarPeDominioTx(
    tx: Prisma.TransactionClient,
    dominio: { personagemSessaoId: number | null; npcSessaoId: number | null },
    custo: number,
  ) {
    if (dominio.personagemSessaoId) {
      const p = await tx.personagemSessao.findUnique({
        where: { id: dominio.personagemSessaoId },
        include: { personagemCampanha: { select: { peAtual: true } } },
      });
      if (!p || p.personagemCampanha.peAtual < custo)
        throw new BusinessException(
          'PE insuficiente.',
          'DOMINIO_PE_INSUFICIENTE',
        );
      await tx.personagemCampanha.update({
        where: { id: p.personagemCampanhaId },
        data: { peAtual: { decrement: custo } },
      });
    }
    if (dominio.npcSessaoId) {
      const n = await tx.npcAmeacaSessao.findUnique({
  ...
  private async encerrarDominioTx(
    tx: Prisma.TransactionClient,
    dominio: {
      id: number;
      sessaoId: number;
    ...
    if (dominio.estado === 'ENCERRADO' || dominio.estado === 'COLAPSADO')
      return;
    await tx.dominioSessao.update({
      where: { id: dominio.id },
      data: {
        estado: motivo.includes('COLAPSO') ? 'COLAPSADO' : 'ENCERRADO',
        encerradoEm: new Date(),
        encerradoPorUsuarioId: usuarioId,
        motivoEncerramento: motivo,
      },
    });
    if (dominio.sustentacaoHabilidadeId)
      await tx.personagemSessaoHabilidadeSustentada.updateMany({
        where: { id: dominio.sustentacaoHabilidadeId, ativa: true },
        data: {
          ativa: false,
          desativadaEm: new Date(),
          desativadaPorUsuarioId: usuarioId,
  ...
  private async aplicarAjusteTurnoSessao(
    ...
    return this.prisma.$transaction(async (tx) => {
      await bloquearSessaoTx(tx, campanhaId, sessaoId);
      const sessao = await tx.sessao.findUnique({
        where: { id: sessaoId },
      });
      ...
      if (sessao.rodadaAtual !== dto.rodadaEsperada) {
        throw new SessaoTurnoDesatualizadoException(campanhaId, sessaoId);
      }
      ...
      if (regraAlternada?.ativo) {
        return this.aplicarAjusteLadoIniciativaAlternadaTx(tx, {
          ...
          acesso,
          acao,
          ladoAtualIdEsperado: dto.ladoAtualIdEsperado,
        });
      }
      ...
      if (dto.indiceTurnoEsperado !== indiceAnterior) {
        throw new SessaoTurnoDesatualizadoException(campanhaId, sessaoId);
      }
  ...
  private async processarEfeitosAutomaticosTurnoSessao(
    ...
    try {
      ...
      await this.marcarFalhaEfeitosTurno(processamento).catch(
        (erroMarcacao) => {
          this.logger.error(
            `Falha ao registrar erro dos efeitos automáticos do turno ${processamento.eventoId}.`,
            erroMarcacao instanceof Error ? erroMarcacao.stack : undefined,
          );
        },
      );
      this.logger.error(
        `Falha ao processar efeitos automáticos do turno: ${JSON.stringify({
          ...
          acao: processamento.contexto.acao,
          rodadaAnterior: processamento.contexto.rodadaAnterior,
          rodadaNova: processamento.contexto.rodadaNova,
        })}`,
  ...
  private async processarProximoPassoEfeitosTurnoTx(
    ...
    await bloquearEventoSessaoTx(
      tx,
      processamento.campanhaId,
      processamento.sessaoId,
      processamento.eventoId,
    );
    const evento = await tx.eventoSessao.findUnique({
  ...
  private async executarPassoEfeitosTurnoTx(
    ...
    sessaoId: number,
    contexto: ContextoEfeitosTurnoSessao,
    passo: PassoEfeitosTurnoSessao,
  ): Promise<void> {
    if (passo.tipo === 'SUSTENTACOES_RODADA') {
      await this.cobrarSustentacoesAtivasRodadaTx(tx, {
        sessaoId,
        cenaId: contexto.cenaId,
        rodadaNova: contexto.rodadaNova,
      });
      return;
    }
    if (passo.tipo === 'CONDICOES_RODADA') {
      await this.processarCondicoesNoAvancoTurnoTx(tx, {
        sessaoId,
        cenaId: contexto.cenaId,
        rodadaAnterior: contexto.rodadaAnterior,
        rodadaNova: contexto.rodadaNova,
    ...
    const participante: ParticipanteIniciativa = {
      tipoParticipante: passo.tipoParticipante ?? 'PERSONAGEM',
      token: passo.chave,
      personagemSessaoId: passo.personagemSessaoId ?? null,
      npcSessaoId: passo.npcSessaoId ?? null,
      personagemCampanhaId: null,
      donoId: null,
      nomeJogador: null,
      nomePersonagem: '',
    ...
    await this.processarCondicoesNoAvancoTurnoTx(tx, {
      ...
      rodadaAnterior: contexto.rodadaAnterior,
      rodadaNova: contexto.rodadaNova,
      participanteTurnoNovo: participante,
      processarDuracoesPorRodada: false,
      sincronizarAutomaticasGlobais: false,
    });
  }

  private async persistirContextoEfeitosTurnoTx(
    tx: Prisma.TransactionClient,
    eventoId: number,
    contexto: ContextoEfeitosTurnoSessao,
  ): Promise<void> {
    ...
    await tx.eventoSessao.update({
      ...
      data: {
        dados: this.jsonParaPersistencia({
          ...dados,
          efeitosAutomaticos: contexto,
        }),
      },
    });
  }

  private async marcarFalhaEfeitosTurno(
    processamento: ProcessamentoEfeitosTurnoSessao,
  ): Promise<void> {
    await executarComRetryConcorrencia(
      'registrar falha dos efeitos automaticos do turno',
      () =>
        this.prisma.$transaction(async (tx) => {
          await bloquearSessaoTx(
            tx,
          ...
          const contexto = evento
            ? this.lerContextoEfeitosTurno(evento.dados)
            : null;
          if (!contexto || contexto.status === 'CONCLUIDO') return;
          const indicePasso = contexto.passos.findIndex(
            (passo) => passo.status !== 'CONCLUIDO',
          );
          const passos = contexto.passos.map((passo, indice) =>
            indice === indicePasso
              ? { ...passo, status: 'ERRO' as const }
              : passo,
          );
          const agora = new Date().toISOString();
          await this.persistirContextoEfeitosTurnoTx(
            tx,
            processamento.eventoId,
            {
              ...contexto,
  ...
  private lerContextoEfeitosTurno(
    dadosJson: Prisma.JsonValue | null,
  ): ContextoEfeitosTurnoSessao | null {
    const dados = this.extrairRegistro(dadosJson);
    const registro = this.lerRegistroOpcionalRegistro(
      dados,
      'efeitosAutomaticos',
    );
    if (!registro || registro.versao !== 2) return null;

    const acao = registro.acao;
    const cenaId = registro.cenaId;
    const rodadaAnterior = registro.rodadaAnterior;
    const rodadaNova = registro.rodadaNova;
    if (
      (acao !== 'AVANCAR' && acao !== 'VOLTAR' && acao !== 'PULAR') ||
      typeof cenaId !== 'number' ||
      !Number.isInteger(cenaId) ||
      typeof rodadaAnterior !== 'number' ||
      !Number.isInteger(rodadaAnterior) ||
      typeof rodadaNova !== 'number' ||
      !Number.isInteger(rodadaNova) ||
      !Array.isArray(registro.passos)
    ) {
      return null;
    }

    const passos: PassoEfeitosTurnoSessao[] = [];
    for (const bruto of registro.passos) {
      ...
      if (
        ...
        (passo.tipo !== 'SUSTENTACOES_RODADA' &&
          passo.tipo !== 'CONDICOES_RODADA' &&
          passo.tipo !== 'CONDICOES_PARTICIPANTE')
      ) {
        return null;
      }
      const status = this.normalizarStatusPassoEfeitosTurno(passo.status);
      if (!status) return null;
      const tipoParticipante =
        passo.tipoParticipante === 'PERSONAGEM' ||
        passo.tipoParticipante === 'NPC'
          ? passo.tipoParticipante
          : undefined;
      const personagemSessaoId =
        typeof passo.personagemSessaoId === 'number' &&
        Number.isInteger(passo.personagemSessaoId)
          ? passo.personagemSessaoId
          : null;
      const npcSessaoId =
        typeof passo.npcSessaoId === 'number' &&
        Number.isInteger(passo.npcSessaoId)
          ? passo.npcSessaoId
          : null;
      if (
        passo.tipo === 'CONDICOES_PARTICIPANTE' &&
        (!tipoParticipante ||
          (tipoParticipante === 'PERSONAGEM' && !personagemSessaoId) ||
          (tipoParticipante === 'NPC' && !npcSessaoId))
      ) {
        return null;
      }
      passos.push({
    ...
    const status = this.normalizarStatusPassoEfeitosTurno(registro.status);
    if (!status) return null;
    return {
      versao: 2,
      status,
      ...
      tentativas:
        ...
        Number.isInteger(registro.tentativas)
          ? Math.max(0, registro.tentativas)
          : 0,
      atualizadoEm:
      ...
      ultimaFalhaEm:
        typeof registro.ultimaFalhaEm === 'string'
          ? registro.ultimaFalhaEm
          : undefined,
    };
  }

  private normalizarStatusPassoEfeitosTurno(
    valor: unknown,
  ): StatusPassoEfeitosTurno | null {
    if (valor === 'EM_PROCESSAMENTO') return 'PENDENTE';
    if (valor === 'PENDENTE' || valor === 'ERRO' || valor === 'CONCLUIDO') {
      return valor;
    }
    return null;
  }

  private reconstruirContextoEfeitosTurnoLegado(args: {
    tipoEvento: string;
    cenaId: number | null;
    dados: Prisma.JsonValue | null;
  }): ContextoEfeitosTurnoSessao | null {
    const dados = this.extrairRegistro(args.dados);
    const registro = this.lerRegistroOpcionalRegistro(
      dados,
      'efeitosAutomaticos',
    );
    const status = this.normalizarStatusPassoEfeitosTurno(registro?.status);
    const rodadaAnterior = this.lerInteiroRegistro(dados, 'rodadaAnterior');
    const rodadaNova = this.lerInteiroRegistro(dados, 'rodadaNova');
    if (
      !status ||
      !args.cenaId ||
      rodadaAnterior === null ||
    ...
    const acao: AcaoAjusteTurnoSessao = args.tipoEvento.includes('RECUAD')
      ? 'VOLTAR'
      : args.tipoEvento === 'TURNO_PULADO'
        ? 'PULAR'
        : 'AVANCAR';
    const tokenNovo = this.lerTextoOpcionalRegistro(dados, 'tokenNovo');
    const token = tokenNovo ? this.lerTokenParticipante(tokenNovo) : null;
    if (
      status !== 'CONCLUIDO' &&
      acao !== 'VOLTAR' &&
      args.tipoEvento.startsWith('INICIATIVA_ALTERNADA')
    ) {
      return null;
    }
    const participantesTurnoNovos: ParticipanteIniciativa[] = token
      ? [
          {
            tipoParticipante: token.tipoParticipante,
            token: tokenNovo ?? '',
            personagemSessaoId:
              token.tipoParticipante === 'PERSONAGEM' ? token.id : null,
            npcSessaoId: token.tipoParticipante === 'NPC' ? token.id : null,
            personagemCampanhaId: null,
            donoId: null,
            nomeJogador: null,
            nomePersonagem: '',
    ...
    if (status === 'CONCLUIDO') {
      return {
        ...contexto,
        status: 'CONCLUIDO',
        passos: contexto.passos.map((passo) => ({
          ...passo,
          status: 'CONCLUIDO',
        })),
      };
  ...
  private async obterProcessamentoEfeitosTurnoPorEvento(
    campanhaId: number,
    sessaoId: number,
    eventoId: number,
  ): Promise<ProcessamentoEfeitosTurnoSessao> {
    const evento = await this.prisma.eventoSessao.findFirst({
      where: {
        id: eventoId,
        sessaoId,
        sessao: { campanhaId },
        tipoEvento: { in: [...TIPOS_EVENTO_AJUSTE_TURNO] },
      },
      select: { id: true, cenaId: true, tipoEvento: true, dados: true },
    });
    if (!evento) {
      throw new SessaoEventoNaoEncontradoException(eventoId, sessaoId);
    }
    const contexto =
      this.lerContextoEfeitosTurno(evento.dados) ??
      this.reconstruirContextoEfeitosTurnoLegado(evento);
    if (!contexto) {
      throw new SessaoEfeitosTurnoFalharamException(
        campanhaId,
        sessaoId,
    ...
    return { eventoId, campanhaId, sessaoId, contexto };
  }

  private async obterEfeitosTurnoPendentesTx(
    db: Prisma.TransactionClient | PrismaService,
    sessaoId: number,
  ): Promise<EfeitosTurnoPendentesResumo | null> {
    const eventoDelegate = (
      db as Prisma.TransactionClient & {
        eventoSessao?: Prisma.TransactionClient['eventoSessao'];
      }
    ).eventoSessao;
    if (!eventoDelegate?.findMany) return null;
    const eventos = await eventoDelegate.findMany({
      where: {
        sessaoId,
        tipoEvento: { in: [...TIPOS_EVENTO_AJUSTE_TURNO] },
      },
    ...
    for (const evento of eventos) {
      ...
      if (!contexto) {
        ...
        const status = this.normalizarStatusPassoEfeitosTurno(registro?.status);
        if (!status || status === 'CONCLUIDO') continue;
        return {
          eventoId: evento.id,
          status: 'ERRO',
          acao: evento.tipoEvento.includes('RECUAD')
            ? 'VOLTAR'
            : evento.tipoEvento === 'TURNO_PULADO'
              ? 'PULAR'
              : 'AVANCAR',
          rodadaAnterior: this.lerInteiroRegistro(dados, 'rodadaAnterior') ?? 0,
          rodadaNova: this.lerInteiroRegistro(dados, 'rodadaNova') ?? 0,
          tentativas: registro
            ? (this.lerInteiroRegistro(registro, 'tentativas') ?? 0)
            : 0,
        };
      }
      if (contexto.status === 'CONCLUIDO') continue;
      return {
        eventoId: evento.id,
        status: contexto.status === 'ERRO' ? 'ERRO' : 'PENDENTE',
        acao: contexto.acao,
        rodadaAnterior: contexto.rodadaAnterior,
        rodadaNova: contexto.rodadaNova,
  ...
  private async registrarAcertosGarantidosNoInicioTurnoTx(
    ...
    const personagemSessaoId =
      args.participante.tipoParticipante === 'PERSONAGEM'
        ? args.participante.personagemSessaoId
        : null;
    const npcSessaoId =
      args.participante.tipoParticipante === 'NPC'
        ? args.participante.npcSessaoId
        : null;
    if (!personagemSessaoId && !npcSessaoId) return;
    const defesa = await tx.defesaAntiDominioSessao.findFirst({
      where: {
        sessaoId: args.sessaoId,
        cenaId: args.cenaId,
        ativa: true,
    ...
    const dominios = await tx.dominioSessao.findMany({
      ...
      select: { id: true, nome: true, descricaoAcertoGarantido: true },
    });
    for (const dominio of dominios) {
      await tx.eventoSessao.create({
        data: {
          sessaoId: args.sessaoId,
          cenaId: args.cenaId,
          personagemAlvoId: personagemSessaoId,
          tipoEvento: 'DOMINIO_ACERTO_GARANTIDO',
          dados: this.jsonParaPersistencia({
            dominioId: dominio.id,
            dominioNome: dominio.nome,
            npcAlvoId: npcSessaoId,
            descricaoTecnica: dominio.descricaoAcertoGarantido,
            aviso:
              'Gatilho auditável: resolva o efeito particular pela ficha e rolagem da técnica.',
          }),
        },
      });
    }
  ...
  private descreverEventoSessao(
    ...
    switch (tipoEvento) {
      ...
      case 'TURNO_AVANCADO': {
        ...
        return `Turno avançado${rodada !== null ? ` (rodada ${rodada})` : ''}${this.sufixoStatusEfeitosTurno(dados)}`;
      }
      case 'TURNO_RECUADO': {
        const rodada = this.lerInteiroRegistro(dados, 'rodadaNova');
        return `Turno recuado${rodada !== null ? ` (rodada ${rodada})` : ''}${this.sufixoStatusEfeitosTurno(dados)}`;
      }
      case 'TURNO_PULADO': {
        const rodada = this.lerInteiroRegistro(dados, 'rodadaNova');
        return `Turno pulado${rodada !== null ? ` (rodada ${rodada})` : ''}${this.sufixoStatusEfeitosTurno(dados)}`;
      }
      case 'INICIATIVA_ALTERNADA_AVANCADA':
        return `Lado da iniciativa avançado${this.sufixoStatusEfeitosTurno(dados)}`;
      case 'INICIATIVA_ALTERNADA_RECUADA':
        return `Lado da iniciativa recuado${this.sufixoStatusEfeitosTurno(dados)}`;
      case 'TURNO_DESFEITO':
  ...
  private sufixoStatusEfeitosTurno(dados: Record<string, unknown>): string {
    const efeitos = this.lerRegistroOpcionalRegistro(
      dados,
      'efeitosAutomaticos',
    );
    const status = this.normalizarStatusPassoEfeitosTurno(efeitos?.status);
    if (status === 'ERRO') return ' - falha nos efeitos automáticos';
    if (status === 'PENDENTE') return ' - efeitos automáticos pendentes';
    return '';
  }
