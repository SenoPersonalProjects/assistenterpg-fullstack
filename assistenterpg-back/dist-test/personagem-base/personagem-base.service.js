...
const habilidades_1 = require("src/common/constants/habilidades");
...
let PersonagemBaseService = class PersonagemBaseService {
    ...
    async validarItensInventarioNoPreview(dto, inventarioSomarIntelecto) {
    ...
    async buscarHabilidadesPersonagem(params, prisma = this.prisma) {
        const { nivel, origemId, classeId, trilhaId, caminhoId, estudouEscolaTecnica, poderesGenericos, } = params;
        const habilidades = [];
        ...
        if (estudouEscolaTecnica) {
            const escolaTecnica = await prisma.habilidade.findFirst({
                where: {
                    OR: [
                        { codigo: habilidades_1.HABILIDADE_ESCOLA_TECNICA_CODIGO },
                        { nome: habilidades_1.HABILIDADE_ESCOLA_TECNICA_NOME },
            ...
            if (escolaTecnica) {
                const possuiEfeitoTecnica = escolaTecnica.efeitosGrau?.some((efeito) => efeito.tipoGrauCodigo === 'TECNICA_AMALDICOADA') ?? false;
                ...
                habilidades.push({
                    ...
                    habilidade: mapHabilidade(escolaComEfeito),
                });
            }
        }
    ...
    async preview(donoId, dto) {
        ...
        const inventarioSomarIntelecto = this.calcularModificadoresDerivadosPorHabilidades(estado.habilidades, dtoPreview.nivel).inventarioSomarIntelecto;
        const { itensValidados, errosItens } = await this.validarItensInventarioNoPreview(dtoPreview, inventarioSomarIntelecto);
        return {
