...
const inventario_capacidade_1 = require("../inventario/utils/inventario-capacidade");
...
function getInventarioSomarIntelectoFromMecanicas(mecanicas) {
    if (!isRecord(mecanicas))
        return false;
    const inventario = mecanicas.inventario;
    if (!isRecord(inventario))
        return false;
...
let PersonagemBaseMapper = class PersonagemBaseMapper {
    ...
    async mapDetalhado(personagem, prisma) {
        ...
        const grausAprimoramentoAjustados = (personagem.grausAprimoramento ?? [])
            ...
            .filter((g) => g !== null);
        const inventarioSomarIntelecto = (personagem.habilidadesBase ?? []).some((h) => getInventarioSomarIntelectoFromMecanicas(h.habilidade.mecanicasEspeciais)) ||
            (personagem.poderesGenericos ?? []).some((p) => getInventarioSomarIntelectoFromMecanicas(p.habilidade.mecanicasEspeciais));
        const atributoInventarioBase = (0, inventario_capacidade_1.calcularAtributoBaseInventario)({
            forca: personagem.forca,
            intelecto: personagem.intelecto,
            somarIntelecto: inventarioSomarIntelecto,
        });
        const espacosInventarioBaseFallback = (0, inventario_capacidade_1.calcularEspacosInventarioBase)(atributoInventarioBase);
        const espacosBase = personagem.espacosInventarioBase ?? espacosInventarioBaseFallback;
        ...
        const tecnicasNaoInatas = tecnicasNaoInatasCatalogo
        ...
        const tecnicaInataDetalhada = personagem.tecnicaInata
