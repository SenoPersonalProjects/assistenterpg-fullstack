...
const npc_ameaca_exception_1 = require("src/common/exceptions/npc-ameaca.exception");
const regras_tecnicas_nao_inatas_1 = require("src/personagem-base/regras-criacao/regras-tecnicas-nao-inatas");
const business_exception_1 = require("src/common/exceptions/business.exception");
const TIPOS_EVENTO_REVERSIVEIS = new Set([
    'TURNO_AVANCADO',
    'TURNO_RECUADO',
    'TURNO_PULADO',
...
let SessaoService = class SessaoService {
    ...
    async listarSessoesCampanha(campanhaId, usuarioId) {
        ...
        return sessoes.map((sessao) => {
            const controleTurnosAtivo = !['LIVRE', 'BASE'].includes(sessao.cenaAtualTipo);
            return {
                id: sessao.id,
    ...
    async criarSessaoCampanha(campanhaId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        this.assertMestre(acesso, 'iniciar sessao');
        const titulo = dto.titulo?.trim() || `Sessao ${new Date().toLocaleDateString('pt-BR')}`;
    ...
    async encerrarSessaoCampanha(campanhaId, sessaoId, usuarioId) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
    ...
    async buscarDetalheSessao(campanhaId, sessaoId, usuarioId) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        const personagensOrdenados = sessao.personagens.filter((personagem) => personagem.cenaId === cenaAtualId);
        const npcsCenaAtual = sessao.npcs.filter((npc) => npc.cenaId === cenaAtualId);
        const participantesIniciativaPadrao = this.montarParticipantesIniciativa(personagensOrdenados, npcsCenaAtual, acesso.ehMestre, usuarioId);
        const ordemPersistida = await this.obterOrdemIniciativaPersistida(this.prisma, sessaoId);
        const participantesIniciativa = this.aplicarOrdemIniciativaPersistida(participantesIniciativaPadrao, ordemPersistida);
        ...
        const obterValorIniciativa = (indice, participante) => participante?.iniciativaValor ?? valorIniciativaBase - indice;
        const controleTurnosAtivo = !['LIVRE', 'BASE'].includes(sessao.cenaAtualTipo);
        const indiceTurno = controleTurnosAtivo
            ? this.clampIndiceTurno(sessao.indiceTurnoAtual, totalParticipantesIniciativa)
            : null;
        const personagemTurnoAtual = controleTurnosAtivo &&
        ...
        for (const sustentacao of sessao.habilidadesSustentadas) {
            const listaAtual = sustentacoesPorPersonagemSessao.get(sustentacao.personagemSessaoId) ??
                [];
            listaAtual.push({
                id: sustentacao.id,
        ...
        const tecnicasNaoInatasCatalogo = await this.listarTecnicasNaoInatasCatalogo(this.prisma);
        const personagemCampanhaIds = personagensOrdenados.map((personagem) => personagem.personagemCampanha.id);
        const catalogoPericias = personagemCampanhaIds.length > 0
            ...
            : [];
        const mapaPericiaPorCodigo = new Map(catalogoPericias.map((pericia) => [pericia.codigo, pericia]));
        const mapaPericiaPorBusca = new Map();
        for (const pericia of catalogoPericias) {
            mapaPericiaPorBusca.set(this.normalizarBuscaPericia(pericia.codigo), pericia.codigo);
            mapaPericiaPorBusca.set(this.normalizarBuscaPericia(pericia.nome), pericia.codigo);
        }
        const bonusEquipamentoPorPersonagem = await this.calcularBonusEquipamentoPericias(personagemCampanhaIds, mapaPericiaPorBusca);
        return {
            id: sessao.id,
            campanhaId: sessao.campanhaId,
            titulo: sessao.titulo,
            ...
            turnoAtual: personagemTurnoAtual
                ? {
                    ...
                    valorIniciativa: indiceTurno === null
                        ? null
            ...
            cards: personagensOrdenados.map((personagem) => {
                const podeEditar = acesso.ehMestre || personagem.personagemCampanha.donoId === usuarioId;
                const visibilidade = podeEditar ? 'completa' : 'resumida';
                const tecnicas = this.resolverTecnicasSessaoPersonagem(personagem.personagemCampanha, tecnicasNaoInatasCatalogo);
                const sustentacoesAtivas = sustentacoesPorPersonagemSessao.get(personagem.id) ?? [];
                const condicoesAtivas = this.mapearCondicoesAtivasSessao(personagem.condicoes);
                const bonusEquipamento = bonusEquipamentoPorPersonagem.get(personagem.personagemCampanha.id) ??
                    new Map();
                const periciasBase = personagem.personagemCampanha.personagemBase?.pericias ?? [];
                const mapaPericias = new Map();
                for (const pericia of periciasBase) {
                    if (!pericia.pericia?.codigo)
                        continue;
                ...
                const periciasSessao = Array.from(mapaPericias.values())
                    ...
                    bonusTotal: pericia.bonusTreinamento +
                ...
                const atributosPersonagem = personagem.personagemCampanha.personagemBase
                    ? {
                        agilidade: Number(personagem.personagemCampanha.personagemBase.agilidade ?? 0),
                        forca: Number(personagem.personagemCampanha.personagemBase.forca ?? 0),
                        intelecto: Number(personagem.personagemCampanha.personagemBase.intelecto ?? 0),
                        presenca: Number(personagem.personagemCampanha.personagemBase.presenca ?? 0),
                        vigor: Number(personagem.personagemCampanha.personagemBase.vigor ?? 0),
                    }
                    : null;
                return {
                    ...
                    tecnicasNaoInatas: visibilidade === 'completa' ? tecnicas.tecnicasNaoInatas : [],
                    sustentacoesAtivas: visibilidade === 'completa' ? sustentacoesAtivas : [],
            ...
            npcs: npcsCenaAtual.map((npc) => {
                const atributosNpc = npc.npcAmeaca
                    ...
                    : null;
                const resolverDadosPericiaNpc = (campoDados, atributoBase) => {
                    if (!npc.npcAmeaca || !atributosNpc)
                        return 0;
    ...
    async listarChatSessao(campanhaId, sessaoId, usuarioId, afterId) {
    ...
    async enviarMensagemChatSessao(campanhaId, sessaoId, usuarioId, mensagem) {
        await this.obterSessaoComAcesso(campanhaId, sessaoId, usuarioId);
    ...
    async listarEventosSessao(campanhaId, sessaoId, usuarioId, query) {
        ...
        const ultimoEventoReversivel = acesso.ehMestre
            ...
            : null;
        return eventos.map((evento) => this.mapearEventoSessao(evento, acesso.ehMestre && evento.id === ultimoEventoReversivel?.id));
    }
    async desfazerEventoSessao(campanhaId, sessaoId, eventoId, usuarioId, motivo) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (!evento) {
                throw new campanha_exception_1.SessaoEventoNaoEncontradoException(eventoId, sessaoId, campanhaId);
            }
            if (!TIPOS_EVENTO_REVERSIVEIS.has(evento.tipoEvento)) {
                throw new campanha_exception_1.SessaoEventoDesfazerNaoPermitidoException(eventoId, sessaoId, evento.tipoEvento);
            }
            if (this.eventoJaFoiDesfeito(evento.dados)) {
                throw new campanha_exception_1.SessaoEventoDesfazerNaoPermitidoException(eventoId, sessaoId, evento.tipoEvento);
            }
            const ultimoEventoReversivel = await this.obterUltimoEventoReversivelDisponivel(tx, sessaoId);
            ...
            switch (evento.tipoEvento) {
                ...
                case 'TURNO_RECUADO':
                case 'TURNO_PULADO': {
                    const indiceAnterior = this.lerInteiroRegistro(dadosEvento, 'indiceAnterior');
                    ...
                    const rodadaAnteriorInformada = this.lerInteiroOpcionalRegistro(dadosEvento, 'rodadaAnterior');
                    if (indiceAnterior === null ||
                        indiceNovo === null ||
                        rodadaNova === null) {
                        throw new campanha_exception_1.SessaoEventoDesfazerNaoPermitidoException(eventoId, sessaoId, evento.tipoEvento);
                    }
                    const rodadaAnterior = rodadaAnteriorInformada ??
                ...
                case 'ORDEM_INICIATIVA_ATUALIZADA': {
                    ...
                    const ordemRestaurada = this.aplicarOrdemIniciativaPersistida(participantesPadrao, ordemEvento.ordemAnterior);
                    if (ordemRestaurada.length === 0) {
                        break;
                    }
                    const indiceTurnoRestaurado = this.clampIndiceTurno(ordemEvento.indiceTurnoAnterior, ordemRestaurada.length);
                    await tx.sessao.update({
                        where: { id: sessaoId },
                        data: {
                    ...
                    await tx.eventoSessao.create({
                        data: {
                            ...
                            dados: this.jsonParaPersistencia({
                                eventoOriginalId: evento.id,
                                ordemAtual: ordemRestaurada.map((participante) => participante.token),
                ...
                case 'CENA_ATUALIZADA': {
                    ...
                    const nomeAnterior = this.lerTextoOpcionalRegistro(dadosEvento, 'nomeAnterior');
                    const rodadaAnterior = this.lerInteiroRegistro(dadosEvento, 'rodadaAnterior');
                    const indiceTurnoAnterior = this.lerInteiroRegistro(dadosEvento, 'indiceTurnoAnterior');
                    if (cenaAnteriorId === null ||
                        tipoAnterior === null ||
                        rodadaAnterior === null ||
                ...
                case 'CONDICAO_APLICADA': {
                    const condicaoSessaoId = this.lerInteiroRegistro(dadosEvento, 'condicaoSessaoId');
                    const modoOperacao = this.lerTextoOpcionalRegistro(dadosEvento, 'modoOperacao');
                    ...
                    if (modoOperacao === 'ATUALIZADA') {
                        const snapshotAnterior = this.lerSnapshotCondicaoRegistro(dadosEvento, 'snapshotAnterior');
                        if (!snapshotAnterior) {
                            throw new campanha_exception_1.SessaoEventoDesfazerNaoPermitidoException(eventoId, sessaoId, evento.tipoEvento);
                        }
    ...
    async avancarTurnoSessao(campanhaId, sessaoId, usuarioId) {
        await this.aplicarAjusteTurnoSessao(campanhaId, sessaoId, usuarioId, 'AVANCAR');
        return this.buscarDetalheSessao(campanhaId, sessaoId, usuarioId);
    }
    async voltarTurnoSessao(campanhaId, sessaoId, usuarioId) {
        await this.aplicarAjusteTurnoSessao(campanhaId, sessaoId, usuarioId, 'VOLTAR');
        return this.buscarDetalheSessao(campanhaId, sessaoId, usuarioId);
    }
    async pularTurnoSessao(campanhaId, sessaoId, usuarioId) {
        await this.aplicarAjusteTurnoSessao(campanhaId, sessaoId, usuarioId, 'PULAR');
        return this.buscarDetalheSessao(campanhaId, sessaoId, usuarioId);
    }
    async atualizarOrdemIniciativaSessao(campanhaId, sessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            const ordemAtual = this.aplicarOrdemIniciativaPersistida(participantesPadrao, ordemPersistida);
            const tokensAtuais = ordemAtual.map((participante) => participante.token);
            const tokensNovos = dto.ordem.map((item) => this.criarTokenParticipante(item.tipoParticipante, item.id));
            const ordemNovaNormalizada = this.validarEOrdenarIniciativaPorTokens(ordemAtual, tokensNovos, sessaoId, campanhaId);
            const tokensNovaOrdem = ordemNovaNormalizada.map((participante) => participante.token);
            const indiceAnterior = this.clampIndiceTurno(sessao.indiceTurnoAtual, ordemAtual.length);
            const tokenTurnoAnterior = ordemAtual[indiceAnterior]?.token ?? null;
            const indiceTurnoNovoInformado = typeof dto.indiceTurnoAtual === 'number'
                ? this.clampIndiceTurno(dto.indiceTurnoAtual, ordemNovaNormalizada.length)
                : null;
            const indiceTurnoNovo = tokenTurnoAnterior
                ? this.clampIndiceTurno(ordemNovaNormalizada.findIndex((participante) => participante.token === tokenTurnoAnterior), ordemNovaNormalizada.length)
    ...
    async atualizarCenaSessao(campanhaId, sessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            const limitesCategoriaAtual = sessao.limitesCategoriaInventarioAtivo ?? false;
            const limitesCategoriaAtivo = dto.limitesCategoriaAtivo ??
    ...
    async adicionarPersonagemSessao(campanhaId, sessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        await this.prisma.$transaction(async (tx) => {
            const sessao = await tx.sessao.findUnique({
                where: { id: sessaoId },
            ...
            if (!personagemCampanha) {
                throw new campanha_exception_1.PersonagemCampanhaNaoEncontradoException(dto.personagemCampanhaId, campanhaId);
            }
            if (!acesso.ehMestre && personagemCampanha.donoId !== usuarioId) {
                throw new campanha_exception_1.CampanhaPersonagemEdicaoNegadaException(campanhaId, personagemCampanha.id, usuarioId);
            }
            const cenaAtual = await this.obterCenaAtualSessaoTx(tx, sessaoId);
            ...
            const iniciativaValor = dto.iniciativaValor !== undefined ? dto.iniciativaValor : null;
            if (personagemSessaoAtual) {
                await tx.personagemSessao.update({
                    ...
                    data: {
                        cenaId: cenaAtual.id,
                        iniciativaValor: iniciativaValor ?? personagemSessaoAtual.iniciativaValor ?? null,
                    },
                });
            }
            else {
    ...
    async removerPersonagemSessao(campanhaId, sessaoId, personagemSessaoId, usuarioId) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        await this.prisma.$transaction(async (tx) => {
            const sessao = await tx.sessao.findUnique({
                where: { id: sessaoId },
            ...
            if (!personagemSessao) {
                throw new campanha_exception_1.PersonagemSessaoNaoEncontradoException(personagemSessaoId, sessaoId, campanhaId);
            }
            if (!acesso.ehMestre &&
                personagemSessao.personagemCampanha.donoId !== usuarioId) {
                throw new campanha_exception_1.CampanhaPersonagemEdicaoNegadaException(campanhaId, personagemSessao.personagemCampanha.id, usuarioId);
            }
            await tx.personagemSessao.update({
                where: {
                    id: personagemSessaoId,
    ...
    async atualizarValorIniciativaSessao(campanhaId, sessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        await this.prisma.$transaction(async (tx) => {
            const sessao = await tx.sessao.findUnique({
                where: { id: sessaoId },
            ...
            if (dto.tipoParticipante === 'NPC') {
                ...
                if (!npc) {
                    throw new campanha_exception_1.NpcSessaoNaoEncontradoException(dto.id, sessaoId, campanhaId);
                }
                await tx.npcAmeacaSessao.update({
                    where: { id: dto.id },
                    data: {
                        iniciativaValor: dto.valorIniciativa !== undefined ? dto.valorIniciativa : null,
                    },
                });
                return;
            }
            ...
            if (!personagemSessao) {
                throw new campanha_exception_1.PersonagemSessaoNaoEncontradoException(dto.id, sessaoId, campanhaId);
            }
            if (!acesso.ehMestre &&
                personagemSessao.personagemCampanha.donoId !== usuarioId) {
                throw new campanha_exception_1.CampanhaPersonagemEdicaoNegadaException(campanhaId, personagemSessao.personagemCampanha.id, usuarioId);
    ...
    async adicionarNpcSessao(campanhaId, sessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (!npcBase) {
                throw new npc_ameaca_exception_1.NpcAmeacaNaoEncontradaException(dto.npcAmeacaId);
            }
            const cenaAtual = await this.obterCenaAtualSessaoTx(tx, sessaoId);
            const pontosVidaMax = dto.pontosVidaMax ?? npcBase.pontosVida;
            const pontosVidaAtual = this.clampNumero(dto.pontosVidaAtual ?? pontosVidaMax, 0, pontosVidaMax);
            const sanidade = this.resolverRecursoOpcional(dto.sanAtual, dto.sanMax, null, null);
            const energiaAmaldicoada = this.resolverRecursoOpcional(dto.eaAtual, dto.eaMax, null, null);
            ...
            const npcSessao = await tx.npcAmeacaSessao.create({
                data: {
                    ...
                    iniciativaValor: dto.iniciativaValor === undefined ? null : dto.iniciativaValor,
                    ...
                    machucado: dto.machucado === undefined ? npcBase.machucado : dto.machucado,
    ...
    async atualizarNpcSessao(campanhaId, sessaoId, npcSessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (!npcSessaoAtual) {
                throw new campanha_exception_1.NpcSessaoNaoEncontradoException(npcSessaoId, sessaoId, campanhaId);
            }
            const pontosVidaMax = dto.pontosVidaMax ?? npcSessaoAtual.pontosVidaMax;
            const pontosVidaAtual = this.clampNumero(dto.pontosVidaAtual ?? npcSessaoAtual.pontosVidaAtual, 0, pontosVidaMax);
            const sanidade = this.resolverRecursoOpcional(dto.sanAtual, dto.sanMax, npcSessaoAtual.sanAtual, npcSessaoAtual.sanMax);
            const energiaAmaldicoada = this.resolverRecursoOpcional(dto.eaAtual, dto.eaMax, npcSessaoAtual.eaAtual, npcSessaoAtual.eaMax);
            const data = {
                pontosVidaMax,
                pontosVidaAtual,
            };
    ...
    async removerNpcSessao(campanhaId, sessaoId, npcSessaoId, usuarioId) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
    ...
    async usarHabilidadeSessao(campanhaId, sessaoId, personagemSessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        await this.prisma.$transaction(async (tx) => {
            const sessao = await tx.sessao.findUnique({
                where: { id: sessaoId },
            ...
            if (sessao.status === 'ENCERRADA') {
                throw new business_exception_1.BusinessException('Sessao encerrada nao permite uso de habilidades', 'SESSAO_ENCERRADA', {
                    campanhaId,
                    sessaoId,
                });
            }
            ...
            if (!personagemSessao) {
                throw new business_exception_1.BusinessException('Personagem nao encontrado nesta sessao', 'SESSAO_PERSONAGEM_NOT_FOUND', {
                    campanhaId,
                    sessaoId,
                    personagemSessaoId,
                });
            ...
            const tecnicasNaoInatasCatalogo = await this.listarTecnicasNaoInatasCatalogo(tx);
            const tecnicasDisponiveis = this.resolverTecnicasSessaoPersonagem(personagemSessao.personagemCampanha, tecnicasNaoInatasCatalogo);
            const habilidade = this.buscarHabilidadeTecnicaDisponivel(tecnicasDisponiveis, dto.habilidadeTecnicaId);
            if (!habilidade) {
                throw new business_exception_1.BusinessException('Habilidade nao disponivel para este personagem', 'SESSAO_HABILIDADE_NAO_DISPONIVEL', {
                    campanhaId,
                    sessaoId,
                    personagemSessaoId,
            ...
            const custo = this.resolverCustoUsoHabilidade(habilidade, this.montarMapaGrausPersonagemSessao(personagemSessao.personagemCampanha), dto.variacaoHabilidadeId, dto.acumulos ?? 0);
            if (custo.isSustentada) {
                const sustentacaoExistente = await tx.personagemSessaoHabilidadeSustentada.findFirst({
                    where: {
                        sessaoId,
                        personagemSessaoId,
                ...
                if (sustentacaoExistente) {
                    throw new business_exception_1.BusinessException('Habilidade j\u00e1 sustentada nesta sess\u00e3o', 'SESSAO_SUSTENTACAO_DUPLICADA', {
                        campanhaId,
                        sessaoId,
                        personagemSessaoId,
                        habilidadeTecnicaId: habilidade.id,
            ...
            if (recursosAtuais.eaAtual < custo.custoEA ||
                recursosAtuais.peAtual < custo.custoPE) {
                throw new business_exception_1.BusinessException('Recursos insuficientes para usar esta habilidade', 'SESSAO_RECURSO_INSUFICIENTE', {
                    campanhaId,
                    sessaoId,
                    personagemSessaoId,
                    habilidadeTecnicaId: dto.habilidadeTecnicaId,
            ...
            const limitePeEaPorTurno = Math.max(0, Math.trunc(recursosAtuais.limitePeEaPorTurno ?? 0));
            const custoTotalUso = custo.custoEA + custo.custoPE;
            let gastoPeEaNoTurnoAntesUso = 0;
            if (turnoReferencia) {
                gastoPeEaNoTurnoAntesUso = await this.calcularGastoPeEaNoTurnoAtual(tx, sessaoId, personagemSessaoId, turnoReferencia);
                const bloqueadoPorLimiteTurno = this.deveBloquearPorLimitePeEaTurno(limitePeEaPorTurno, gastoPeEaNoTurnoAntesUso, custoTotalUso, custo.isUsoBaseSemEscalonamento);
                if (bloqueadoPorLimiteTurno) {
                    throw new business_exception_1.BusinessException('Limite de PE/EA por turno excedido para esta acao', 'SESSAO_LIMITE_PEEA_EXCEDIDO', {
                        campanhaId,
                        sessaoId,
                        personagemSessaoId,
                        habilidadeTecnicaId: dto.habilidadeTecnicaId,
    ...
    async aplicarCondicaoSessao(campanhaId, sessaoId, usuarioId, dto) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (!condicao) {
                throw new business_exception_1.BusinessException('Condicao nao encontrada', 'SESSAO_CONDICAO_NOT_FOUND', {
                    campanhaId,
                    sessaoId,
            ...
            const duracao = this.resolverDuracaoCondicao(dto.duracaoModo, dto.duracaoValor);
            const cenaAtualId = sessao.cenas[0]?.id ?? alvo.cenaId;
            if (!cenaAtualId) {
                throw new business_exception_1.BusinessException('Cena atual da sessao nao encontrada para aplicar condicao', 'SESSAO_CENA_ATUAL_NOT_FOUND', {
                    campanhaId,
                    sessaoId,
                });
            }
    ...
    async removerCondicaoSessao(campanhaId, sessaoId, condicaoSessaoId, usuarioId, motivo) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (!condicaoSessao) {
                throw new business_exception_1.BusinessException('Condicao ativa nao encontrada nesta sessao', 'SESSAO_CONDICAO_ATIVA_NOT_FOUND', {
                    campanhaId,
                    sessaoId,
                    condicaoSessaoId,
                });
            ...
            const alvoNome = condicaoSessao.personagemSessao?.personagemCampanha.nome ??
    ...
    async encerrarSustentacaoHabilidadeSessao(campanhaId, sessaoId, personagemSessaoId, sustentacaoId, usuarioId, motivo) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        const motivoLimpo = motivo?.trim() || null;
        await this.prisma.$transaction(async (tx) => {
            const sessao = await tx.sessao.findUnique({
            ...
            if (!sustentacao) {
                throw new business_exception_1.BusinessException('Sustentacao ativa nao encontrada', 'SESSAO_SUSTENTACAO_NOT_FOUND', {
                    campanhaId,
                    sessaoId,
                    personagemSessaoId,
                    sustentacaoId,
    ...
    async validarAcessoSessao(campanhaId, sessaoId, usuarioId) {
        await this.obterSessaoComAcesso(campanhaId, sessaoId, usuarioId);
    }
    async obterAcessoCampanha(campanhaId, usuarioId) {
    ...
    async obterSessaoComAcesso(campanhaId, sessaoId, usuarioId) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        const sessao = await this.prisma.sessao.findUnique({
            where: { id: sessaoId },
            select: {
    ...
    async obterCenaAtualSessaoTx(tx, sessaoId) {
    ...
    assertMestre(acesso, acao) {
        if (!acesso.ehMestre) {
            throw new campanha_exception_1.CampanhaApenasMestreException(acao);
        }
    }
    ...
    resolverEfeitoEscalonamento(escalonamentoEfeito, escalonamentoDano) {
        if (escalonamentoEfeito &&
    ...
    montarResumoEscalonamento(tipoEscalonamento, efeitoEscalonamento, acumulos) {
        if (acumulos <= 0 || !efeitoEscalonamento)
            return null;
        ...
        switch (tipoEscalonamento) {
            ...
            case 'REGRAS': {
                const incrementoRegras = this.lerInteiroRegistro(efeito, 'incrementoRegras') ?? 1;
                const totalRegras = incrementoRegras * acumulos;
                return `Regras adicionais: +${totalRegras}`;
            }
            case 'OUTRO':
            default: {
                const descricao = this.lerTextoOpcionalRegistro(efeito, 'descricaoPorAcumulo') ??
    ...
    mapearHabilidadeTecnicaResumo(habilidade, grausMap) {
        if (!(0, regras_tecnicas_nao_inatas_1.atendeRequisitosGraus)(habilidade.requisitos, grausMap)) {
            return null;
        }
        const tipoGrauEscalonamento = habilidade.grauTipoGrauCodigo?.trim() || null;
        const acumulosMaximosHabilidade = habilidade.escalonaPorGrau && tipoGrauEscalonamento
        ...
        const variacoes = (habilidade.variacoes ?? [])
            ...
            .map((variacao) => {
            const variacaoEscalonavel = typeof variacao.escalonaPorGrau === 'boolean'
                ? variacao.escalonaPorGrau
                : habilidade.escalonaPorGrau;
            const acumulosMaximosVariacao = variacaoEscalonavel && tipoGrauEscalonamento
                ? Math.max(0, grausMap.get(tipoGrauEscalonamento) ?? 0)
                : 0;
            const tipoEscalonamentoVariacao = this.normalizarTipoEscalonamento(variacao.escalonamentoTipo, variacao.escalonamentoDano);
            const efeitoEscalonamentoVariacao = this.resolverEfeitoEscalonamento(variacao.escalonamentoEfeito, variacao.escalonamentoDano);
            return {
                id: variacao.id,
        ...
        const tipoEscalonamentoHabilidade = this.normalizarTipoEscalonamento(habilidade.escalonamentoTipo, habilidade.escalonamentoDano);
        const efeitoEscalonamentoHabilidade = this.resolverEfeitoEscalonamento(habilidade.escalonamentoEfeito, habilidade.escalonamentoDano);
        return {
            id: habilidade.id,
    ...
    filtrarTecnicaPorGrausSessao(tecnica, grausMap) {
        if (tecnica.tipo === 'NAO_INATA' &&
            !(0, regras_tecnicas_nao_inatas_1.atendeRequisitoBaseTecnicaNaoInata)(tecnica.codigo, grausMap)) {
            return null;
        }
    ...
    async listarTecnicasNaoInatasCatalogo(prismaLike) {
    ...
    resolverTecnicasSessaoPersonagem(personagemCampanha, tecnicasNaoInatasCatalogo = []) {
        ...
        const tecnicaInata = personagemCampanha.tecnicaInata
            ...
            : null;
        const mapaTecnicas = new Map();
        for (const tecnicaCatalogo of tecnicasNaoInatasCatalogo) {
            if (!tecnicaCatalogo || tecnicaCatalogo.tipo !== 'NAO_INATA')
                continue;
    ...
    buscarHabilidadeTecnicaDisponivel(tecnicasDisponiveis, habilidadeTecnicaId) {
        const listaTecnicas = [
        ...
        for (const tecnica of listaTecnicas) {
            const habilidade = tecnica.habilidades.find((item) => item.id === habilidadeTecnicaId);
            if (habilidade)
                return habilidade;
        }
        return null;
    }
    resolverCustoUsoHabilidade(habilidade, grausMap, variacaoHabilidadeId, acumulosSolicitados = 0) {
        const variacaoSelecionada = typeof variacaoHabilidadeId === 'number'
        ...
        if (typeof variacaoHabilidadeId === 'number' && !variacaoSelecionada) {
            throw new business_exception_1.BusinessException('Variacao da habilidade nao encontrada', 'SESSAO_VARIACAO_HABILIDADE_NOT_FOUND', {
                habilidadeTecnicaId: habilidade.id,
                variacaoHabilidadeId,
            });
        }
        const acumulosNormalizados = Math.max(0, Math.trunc(Number.isFinite(acumulosSolicitados) ? acumulosSolicitados : 0));
        ...
        const acumulosMaximos = podeEscalonar && tipoGrauEscalonamento
            ? Math.max(0, grausMap.get(tipoGrauEscalonamento) ?? 0)
            : 0;
        if (acumulosNormalizados > 0 && !podeEscalonar) {
            throw new business_exception_1.BusinessException('Esta habilidade nao possui escalonamento por acumulos', 'SESSAO_HABILIDADE_SEM_ESCALONAMENTO', {
                habilidadeTecnicaId: habilidade.id,
        ...
        if (acumulosNormalizados > acumulosMaximos) {
            throw new business_exception_1.BusinessException('Quantidade de acumulos excede o grau de aprimoramento permitido', 'SESSAO_ACUMULO_EXCEDE_GRAU', {
                habilidadeTecnicaId: habilidade.id,
                variacaoHabilidadeId: variacaoSelecionada?.id ?? null,
                acumulosSolicitados: acumulosNormalizados,
                acumulosMaximos,
        ...
        if (acumulosNormalizados > 0) {
            const fallbackEscalonamentoEA = tipoGrauEscalonamento === 'TECNICA_REVERSA' ? 2 : 1;
            custoEscalonamentoEA = this.normalizarCustoPositivo(escalonamentoCustoEA, fallbackEscalonamentoEA);
        ...
        const isUsoBaseSemEscalonamento = variacaoSelecionada === null && acumulosNormalizados === 0;
        ...
        const custoSustentacaoPENormalizado = isSustentada
            ...
            : null;
        const tipoEscalonamentoNormalizado = this.normalizarTipoEscalonamento(escalonamentoTipo, escalonamentoDano);
        const efeitoEscalonamentoNormalizado = this.resolverEfeitoEscalonamento(escalonamentoEfeito, escalonamentoDano);
        const resumoEscalonamento = this.montarResumoEscalonamento(tipoEscalonamentoNormalizado, efeitoEscalonamentoNormalizado, acumulosNormalizados);
        return {
    ...
    montarMapaGrausPersonagemSessao(personagemCampanha) {
        ...
        return (0, regras_tecnicas_nao_inatas_1.montarMapaGraus)(grausPreferenciais.map((grau) => ({
            tipoGrauCodigo: grau.tipoGrau.codigo,
            valor: grau.valor,
        })));
    }
    montarReferenciaTurnoAtualSessao(sessao) {
        if (sessao.cenaAtualTipo === 'LIVRE')
            return null;
        return `${sessao.rodadaAtual}:${sessao.indiceTurnoAtual}`;
    }
    async calcularGastoPeEaNoTurnoAtual(tx, sessaoId, personagemSessaoId, turnoReferencia) {
        ...
        for (const evento of eventos) {
            ...
            if (turnoEvento !== turnoReferencia)
                continue;
            if (this.eventoJaFoiDesfeito(evento.dados))
                continue;
    ...
    deveBloquearPorLimitePeEaTurno(limitePeEaPorTurno, gastoPeEaNoTurnoAntesUso, custoTotalUso, isUsoBaseSemEscalonamento) {
        const limiteNormalizado = Math.max(0, Math.trunc(Number.isFinite(limitePeEaPorTurno) ? limitePeEaPorTurno : 0));
        if (limiteNormalizado <= 0)
            return false;
        const gastoAtual = Math.max(0, Math.trunc(Number.isFinite(gastoPeEaNoTurnoAntesUso)
            ? gastoPeEaNoTurnoAntesUso
            : 0));
        const custoUso = Math.max(0, Math.trunc(Number.isFinite(custoTotalUso) ? custoTotalUso : 0));
    ...
    mapearParticipantesCampanha(campanha) {
        ...
        for (const membro of campanha.membros) {
            if (membro.usuarioId === campanha.dono.id)
                continue;
    ...
    resolverRecursoOpcional(atualDto, maxDto, atualBase, maxBase) {
        ...
        const atual = typeof atualBruto === 'number'
            ? this.clampNumero(atualBruto, 0, max)
            : max;
        return { atual, max };
    }
    clampIndiceTurno(indice, totalPersonagens) {
        if (totalPersonagens <= 0)
            return 0;
        if (indice < 0)
            return 0;
        if (indice >= totalPersonagens)
            return totalPersonagens - 1;
        return indice;
    }
    criarTokenParticipante(tipoParticipante, id) {
        return `${tipoParticipante}:${id}`;
    }
    lerTokenParticipante(token) {
    ...
    lerListaTokensRegistro(registro, chave) {
    ...
    lerOrdemIniciativaEvento(registro) {
        ...
        const indiceTurnoAnterior = this.lerInteiroRegistro(registro, 'indiceTurnoAnterior');
    ...
    montarParticipantesIniciativa(personagens, npcs, ehMestre, usuarioId) {
        const participantesPersonagens = personagens.map((personagem) => {
        ...
        const participantesNpcs = npcs.map((npc) => ({
    ...
    aplicarOrdemIniciativaPersistida(participantesPadrao, tokensPersistidos) {
        if (!tokensPersistidos || tokensPersistidos.length === 0) {
            return participantesPadrao;
        }
        const porToken = new Map(participantesPadrao.map((participante) => [
            participante.token,
            participante,
        ]));
        ...
        for (const token of tokensPersistidos) {
            ...
            if (!participante || tokensUsados.has(token))
                continue;
            tokensUsados.add(token);
            ordenados.push(participante);
        }
        for (const participante of participantesPadrao) {
            if (tokensUsados.has(participante.token))
                continue;
            ordenados.push(participante);
        }
        return ordenados;
    }
    validarEOrdenarIniciativaPorTokens(participantesAtuais, tokensRecebidos, sessaoId, campanhaId) {
        const tokensAtuais = participantesAtuais.map((participante) => participante.token);
        const mapaAtuais = new Map(participantesAtuais.map((participante) => [
            participante.token,
            participante,
        ]));
        const tokensRecebidosUnicos = Array.from(new Set(tokensRecebidos));
        if (tokensRecebidosUnicos.length !== tokensAtuais.length ||
            tokensRecebidosUnicos.some((token) => !mapaAtuais.has(token))) {
            throw new campanha_exception_1.SessaoOrdemIniciativaInvalidaException(sessaoId, campanhaId);
        }
        return tokensRecebidosUnicos.map((token) => mapaAtuais.get(token));
    }
    async obterOrdemIniciativaPersistida(tx, sessaoId) {
        const eventos = await tx.eventoSessao.findMany({
            where: {
                sessaoId,
                tipoEvento: {
        ...
        for (const evento of eventos) {
            if (this.eventoJaFoiDesfeito(evento.dados))
                continue;
            const dados = this.extrairRegistro(evento.dados);
            const tokens = this.lerListaTokensRegistro(dados, 'ordemAtual') ??
    ...
    async carregarParticipantesIniciativa(tx, sessaoId, cenaAtualId, ehMestre, usuarioId) {
        ...
        return this.montarParticipantesIniciativa(personagens, npcs, ehMestre, usuarioId);
    }
    async aplicarAjusteTurnoSessao(campanhaId, sessaoId, usuarioId, acao) {
        const acesso = await this.obterAcessoCampanha(campanhaId, usuarioId);
        this.assertMestre(acesso, acao === 'VOLTAR'
            ? 'voltar turno'
            : acao === 'PULAR'
                ? 'pular turno'
                : 'avancar turno');
        await this.prisma.$transaction(async (tx) => {
            ...
            const participantesPadrao = await this.carregarParticipantesIniciativa(tx, sessaoId, cenaAtual.id, acesso.ehMestre, usuarioId);
            const ordemPersistida = await this.obterOrdemIniciativaPersistida(tx, sessaoId);
            const participantes = this.aplicarOrdemIniciativaPersistida(participantesPadrao, ordemPersistida);
            if (participantes.length === 0) {
                return;
            }
            const indiceAnterior = this.clampIndiceTurno(sessao.indiceTurnoAtual, participantes.length);
            let indiceNovo = indiceAnterior;
            let rodadaNova = sessao.rodadaAtual;
            if (acao === 'VOLTAR') {
                indiceNovo =
                    (indiceAnterior - 1 + participantes.length) % participantes.length;
                rodadaNova = this.clampNumero(sessao.rodadaAtual - (indiceAnterior === 0 ? 1 : 0), 1, Number.MAX_SAFE_INTEGER);
            }
            else {
            ...
            if (rodadaNova > sessao.rodadaAtual) {
                ...
                const eaAtualPorPersonagemCampanha = new Map();
                const peAtualPorPersonagemCampanha = new Map();
                for (const sustentacao of sustentacoesAtivas) {
                    if (sustentacao.ultimaCobrancaRodada >= rodadaNova) {
                        continue;
                    }
                    const personagemCampanhaId = sustentacao.personagemSessao.personagemCampanha.id;
                    const eaAtual = eaAtualPorPersonagemCampanha.get(personagemCampanhaId) ??
                        sustentacao.personagemSessao.personagemCampanha.eaAtual;
                    const peAtual = peAtualPorPersonagemCampanha.get(personagemCampanhaId) ??
                        sustentacao.personagemSessao.personagemCampanha.peAtual;
                    const custoSustentacaoEA = this.normalizarCustoPositivo(sustentacao.custoSustentacaoEA, 1);
                    const custoSustentacaoPE = this.normalizarCustoPositivo(sustentacao.custoSustentacaoPE, 0);
                    if (eaAtual >= custoSustentacaoEA && peAtual >= custoSustentacaoPE) {
                        ...
                        const recursosInsuficientes = [];
                        if (eaAtual < custoSustentacaoEA)
                            recursosInsuficientes.push('EA');
                        if (peAtual < custoSustentacaoPE)
                            recursosInsuficientes.push('PE');
    ...
    mapearCondicoesAtivasSessao(condicoes) {
    ...
    snapshotCondicaoSessao(condicao) {
        return {
            id: condicao.id,
    ...
    resolverDuracaoCondicao(duracaoModo, duracaoValor) {
        const modo = this.normalizarTextoComparacao(duracaoModo) ||
        ...
        if (modo !== CONDICAO_DURACAO_MODOS.ATE_REMOVER &&
            ...
            throw new business_exception_1.BusinessException('Modo de duracao de condicao invalido', 'SESSAO_CONDICAO_DURACAO_INVALIDA', { duracaoModo });
        }
        if (modo === CONDICAO_DURACAO_MODOS.ATE_REMOVER) {
            return {
                duracaoModo: modo,
        ...
        const valor = Number.isFinite(duracaoValor ?? NaN)
            ? Math.max(1, Math.trunc(duracaoValor))
            : null;
        if (!valor) {
            throw new business_exception_1.BusinessException('Duracao numerica da condicao e obrigatoria', 'SESSAO_CONDICAO_DURACAO_VALOR_REQUIRED', { duracaoModo: modo });
        }
        return {
            duracaoModo: modo,
            duracaoValor: valor,
    ...
    async resolverAlvoCondicaoSessaoTx(tx, sessaoId, dto) {
        if (dto.alvoTipo === 'PERSONAGEM') {
            ...
            if (!personagem) {
                throw new business_exception_1.BusinessException('Personagem da sessao nao encontrado para aplicar condicao', 'SESSAO_CONDICAO_ALVO_PERSONAGEM_NOT_FOUND', {
                    sessaoId,
        ...
        if (!npc) {
            throw new business_exception_1.BusinessException('NPC/Ameaca da sessao nao encontrado para aplicar condicao', 'SESSAO_CONDICAO_ALVO_NPC_NOT_FOUND', {
                sessaoId,
    ...
    async sincronizarCondicoesAutomaticasSessao(sessaoId) {
        await this.prisma.$transaction(async (tx) => {
            await this.sincronizarCondicoesAutomaticasSessaoTx(tx, sessaoId);
        });
    }
    async obterMapaCondicoesSistemaTx(tx) {
        const condicaoDelegate = tx.condicao;
        ...
        const resolverPorAliases = (aliases) => {
    ...
    async sincronizarCondicaoAutomaticaAlvoTx(tx, args) {
        if (!args.condicaoId)
            return;
        const existente = await tx.condicaoPersonagemSessao.findFirst({
            where: {
    ...
    async desativarCondicaoAtivaSessaoTx(tx, args) {
        if (!args.condicaoId)
            return;
        const existente = await tx.condicaoPersonagemSessao.findFirst({
            where: {
    ...
    async sincronizarCondicoesAutomaticasSessaoTx(tx, sessaoId) {
        const sessao = await tx.sessao.findUnique({
            where: { id: sessaoId },
            select: {
                id: true,
        ...
        const bloqueios = condicoesBloqueantes.length > 0
            ...
            : [];
        const personagensMortos = new Set();
        const personagensInsanos = new Set();
        const npcsMortos = new Set();
        const npcsInsanos = new Set();
        ...
        for (const npc of npcs) {
            ...
            const morrendo = pvAtual <= 0 && !morto;
            const caido = pvAtual <= 0;
            const enlouquecendo = typeof npc.sanAtual === 'number' && npc.sanAtual <= 0 && !insano;
    ...
    async processarCondicoesNoAvancoTurnoTx(tx, args) {
        const { sessaoId, cenaId, rodadaAnterior, rodadaNova, participanteTurnoNovo, } = args;
        if (!participanteTurnoNovo)
            return;
        await this.sincronizarCondicoesAutomaticasSessaoTx(tx, sessaoId);
        const condicaoSessaoDelegate = tx.condicaoPersonagemSessao;
            ...
            if (personagemSessaoId) {
                ...
                if (personagem) {
                    ...
                    for (const condicao of condicoesAutomaticas) {
                        ...
                        if (condicao.chaveAutomacao === CONDICAO_AUTOMACAO_CHAVES.MORRENDO &&
    ...
    extrairCodigosPericiaBonificada(valor, mapaPorBusca) {
        if (!valor)
            return [];
    ...
    async calcularBonusEquipamentoPericias(personagemCampanhaIds, mapaPorBusca) {
        ...
        for (const item of itensEquipados) {
            const codigos = this.extrairCodigosPericiaBonificada(item.equipamento.periciaBonificada, mapaPorBusca);
            if (codigos.length === 0)
                continue;
            const quantidade = Math.max(1, item.quantidade ?? 1);
            const bonusEquipamento = (item.equipamento.bonusPericia ?? 0) * quantidade;
            ...
            for (const mod of item.modificacoes) {
                ...
                const bonusPericia = efeitos.bonusPericia;
    ...
    jsonParaPersistencia(valor) {
        if (valor === null)
            return client_1.Prisma.JsonNull;
        return valor;
    }
    async obterUltimoEventoReversivelDisponivel(tx, sessaoId) {
        const eventos = await tx.eventoSessao.findMany({
            where: {
                sessaoId,
                tipoEvento: {
    ...
    eventoJaFoiDesfeito(dados) {
        const registro = this.extrairRegistro(dados);
        return registro.desfeito === true;
    }
    extrairRegistro(dados) {
        if (!dados || typeof dados !== 'object' || Array.isArray(dados)) {
            return {};
        }
        return dados;
    }
    lerInteiroRegistro(registro, chave) {
        const valor = registro[chave];
    ...
    lerInteiroOpcionalRegistro(registro, chave) {
        const valor = registro[chave];
    ...
    lerTextoRegistro(registro, chave) {
        const valor = registro[chave];
    ...
    lerTextoOpcionalRegistro(registro, chave) {
        const valor = registro[chave];
        if (valor === undefined || valor === null)
            return null;
        if (typeof valor !== 'string')
    ...
    lerRegistroOpcionalRegistro(registro, chave) {
        const valor = registro[chave];
    ...
    lerSnapshotNpcRegistro(registro, chave) {
        ...
        return {
            ...
            machucado,
            deslocamentoMetros,
            passivasGuia: (bruto.passivasGuia ?? null),
            acoesGuia: (bruto.acoesGuia ?? null),
            notasCena,
            cenaId,
        };
    }
    lerSnapshotCondicaoRegistro(registro, chave) {
        const bruto = this.lerRegistroOpcionalRegistro(registro, chave);
        if (!bruto)
            return null;
    ...
    montarUpdateCondicaoPorSnapshot(snapshot) {
        if (!snapshot) {
            throw new business_exception_1.BusinessException('Snapshot de condicao invalido', 'SESSAO_CONDICAO_SNAPSHOT_INVALIDO');
        }
        return {
    ...
    montarUpdateNpcPorSnapshot(snapshot) {
    ...
    normalizarTipoFichaNpcAmeaca(valor) {
        if (!valor)
            return null;
        const tiposValidos = ['NPC', 'AMEACA'];
        return tiposValidos.includes(valor)
            ? valor
            : null;
    }
    normalizarTipoNpcAmeaca(valor) {
        if (!valor)
            return null;
        const tiposValidos = [
            'HUMANO',
    ...
    async marcarEventoComoDesfeitoTx(tx, evento, usuarioId, motivo) {
    ...
    mapearEventoSessao(evento, podeDesfazer) {
        const dados = this.extrairRegistro(evento.dados);
    ...
    descreverEventoSessao(tipoEvento, dados) {
        switch (tipoEvento) {
            ...
            case 'HABILIDADE_USADA': {
                ...
                const resumoEscalonamento = this.lerTextoOpcionalRegistro(dados, 'resumoEscalonamento');
            ...
            case 'HABILIDADE_SUSTENTADA_COBRADA': {
                ...
                const partesCusto = [];
                if (custoEA !== null && custoEA > 0)
                    partesCusto.push(`EA -${custoEA}`);
                if (custoPE !== null && custoPE > 0)
                    partesCusto.push(`PE -${custoPE}`);
    ...
    mapearEventoChat(evento) {
        const dados = (evento.dados ?? {});
        const mensagem = typeof dados.mensagem === 'string' ? dados.mensagem : '';
        const apelidoFallback = typeof dados.autorApelido === 'string' ? dados.autorApelido : 'Sistema';
        if (!evento.personagemAtor) {
            return {
                id: evento.id,
                criadoEm: evento.criadoEm,
