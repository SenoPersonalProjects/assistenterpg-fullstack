...
let AuthMailService = AuthMailService_1 = class AuthMailService {
    ...
    transporter = null;
    async enviarRecuperacaoSenha(input) {
        ...
        const html = this.renderTemplate({
            ...
            descricao: 'Recebemos um pedido para redefinir sua senha. Se foi voce, clique no botao abaixo.',
    ...
    async enviarVerificacaoEmail(input) {
        ...
        const html = this.renderTemplate({
            ...
            saudacao: `Ola, ${input.apelido}.`,
            descricao: 'Para concluir seu cadastro no AssistenteRPG, confirme seu email pelo botao abaixo.',
    ...
    async enviarEmail(input) {
        const modo = this.obterModo();
        if (modo === 'console') {
            this.logger.log(`[AUTH_EMAIL][console] para=${input.para} assunto="${input.assunto}"`);
        ...
        if (!transporter) {
            this.logger.warn('[AUTH_EMAIL] Provedor de email nao configurado corretamente. Email nao enviado.');
            return;
        }
    ...
    async getTransporter() {
        ...
        const modo = this.obterModo();
        if (modo === 'ethereal') {
            const conta = await nodemailer_1.default.createTestAccount();
            ...
            this.logger.log(`[AUTH_EMAIL] Ethereal habilitado para testes gratuitos (${conta.user})`);
            return this.transporter;
        }
    ...
    getRemetenteFormatado(modo) {
        ...
        const usarResendPadrao = modo === 'resend' && !fromConfigurado && !usuarioSmtp;
        const email = fromConfigurado &&
    ...
    async enviarEmailResend(input) {
        const apiKey = (process.env.RESEND_API_KEY ?? '').trim();
        if (!apiKey) {
            this.logger.warn('[AUTH_EMAIL][resend] RESEND_API_KEY ausente. Email nao enviado.');
            return;
        }
