...
const inventario_capacidade_1 = require("./utils/inventario-capacidade");
...
const personagemInventarioSelect = client_1.Prisma.validator()({
    forca: true,
...
let InventarioService = class InventarioService {
    ...
    async atualizarEstadoInventario(personagemBaseId, prisma) {
        ...
        if (deltaDefesaEquipamento !== 0 ||
            ...
            for (const personagemCampanha of personagensCampanha) {
                const defesaEquipamentoAtualizada = Math.max(0, (personagemCampanha.defesaEquipamento ?? 0) + deltaDefesaEquipamento);
                const esquivaAtualizada = (personagemCampanha.esquiva ?? 0) + deltaEsquiva;
                const bloqueioAtualizado = (personagemCampanha.bloqueio ?? 0) + deltaBloqueio;
    ...
    async previewItensInventario(dto) {
        try {
            ...
            const espacosBase = (0, inventario_capacidade_1.calcularEspacosInventarioBase)(atributoInventarioBase);
