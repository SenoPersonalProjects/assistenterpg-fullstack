...
const regras_tecnicas_nao_inatas_1 = require("../personagem-base/regras-criacao/regras-tecnicas-nao-inatas");
...
let SessaoService = class SessaoService {
    ...
    async buscarDetalheSessao(campanhaId, sessaoId, usuarioId) {
        ...
        const obterValorIniciativa = (indice) => valorIniciativaBase - indice;
        const controleTurnosAtivo = sessao.cenaAtualTipo !== 'LIVRE';
        ...
        return {
            ...
            turnoAtual: personagemTurnoAtual
                ? {
                    ...
                    valorIniciativa: indiceTurno === null ? null : obterValorIniciativa(indiceTurno),
                }
                : null,
            iniciativa: {
    ...
    async atualizarCenaSessao(campanhaId, sessaoId, usuarioId, dto) {
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            const indiceTurnoNovo = dto.tipo === 'LIVRE' ? 0 : sessao.indiceTurnoAtual;
    ...
    deveBloquearPorLimitePeEaTurno(limitePeEaPorTurno, gastoPeEaNoTurnoAntesUso, custoTotalUso, isUsoBaseSemEscalonamento) {
        ...
        if (limiteNormalizado <= 0)
            return false;
        const gastoAtual = Math.max(0, Math.trunc(Number.isFinite(gastoPeEaNoTurnoAntesUso) ? gastoPeEaNoTurnoAntesUso : 0));
    ...
    aplicarOrdemIniciativaPersistida(participantesPadrao, tokensPersistidos) {
        ...
        const porToken = new Map(participantesPadrao.map((participante) => [participante.token, participante]));
    ...
    validarEOrdenarIniciativaPorTokens(participantesAtuais, tokensRecebidos, sessaoId, campanhaId) {
        ...
        const mapaAtuais = new Map(participantesAtuais.map((participante) => [participante.token, participante]));
