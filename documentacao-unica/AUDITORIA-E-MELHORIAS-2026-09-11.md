...
Munição compatível de armas à distância, tipo de grau para escalonamento de habilidade e perícias adicionais de vinculados deixaram de exigir códigos digitados. Todos usam o catálogo existente com nome, descrição, busca e prevenção de duplicidade quando aplicável. Valores legados continuam reconhecidos pela camada de seleção e são preservados no mesmo formato de persistência.

As habilidades próprias de técnica inata deixaram de expor código e JSON como preenchimento comum. O código agora é sempre gerado internamente na criação e não pode ser alterado por essa edição; execução é escolhida pelo catálogo de ações e requisitos de habilidades e variações reutilizam o editor estruturado, inclusive a regra descritiva “Outro”.

Tags de homebrew deixaram de depender de texto separado por vírgulas: o cadastro usa chips removíveis e permite cadastrar termos descritivos locais. O filtro administrativo de técnicas também deixou de expor código técnico; nome, tipo, hereditariedade e fonte permanecem como filtros de operação.

Acessórios e itens operacionais agora escolhem a perícia bonificada pelo catálogo carregado no próprio formulário. Quando a regra exigir uma referência não catalogada, “Outro” revela um campo local explícito; valores legados permanecem editáveis sem conversão de contrato.

As configurações de poderes genéricos deixaram de expor JSON bruto na ficha-base e nas concessões de campanha. O resumo reutilizável resolve perícias, tipos de grau, habilidades da técnica e vinculados por nome, mantendo configurações desconhecidas em texto descritivo seguro.
...
Os códigos de livro, categoria, subcategoria e artigo passaram a ser imutáveis também no backend. Os DTOs de atualização, os contratos do cliente e o serviço descartam códigos enviados por clientes legados; criação e importação administrativa continuam aceitando um código explícito quando necessário.
...
- [ ] Alterar título, posição ou publicação de livro, categoria, subcategoria e artigo existentes; confirmar que suas URLs e códigos permanecem estáveis.
...
- [ ] Criar ou editar uma arma à distância, escolher munição pelo catálogo e confirmar a compatibilidade da arma sem informar código.
- [ ] Criar ou editar habilidade com escalonamento, escolher o tipo de grau pelo modal e conferir o requisito resultante.
- [ ] Criar ou editar um vinculado, adicionar perícia extra pelo catálogo, confirmar que não há duplicidade e que os pontos são preservados.
- [ ] Criar e editar habilidade própria de técnica inata sem informar código, confirmando que o código gerado permanece estável após salvar.
- [ ] Criar requisito de habilidade e variação própria pelo editor estruturado, incluindo “Outro”, e confirmar a exibição correta na ficha e na sessão.
- [ ] Criar, remover e persistir várias tags em um homebrew sem usar separadores; conferir a busca pela biblioteca.
- [ ] Confirmar que o painel de técnicas continua filtrando por nome, tipo, hereditariedade e fonte sem expor filtro de código.
- [ ] Criar e editar acessório e item operacional, selecionar uma perícia do catálogo e uma referência “Outro”; confirmar que ambos persistem e aparecem corretamente na ficha.
- [ ] Conceder poder com Ritual Predileto, Shikigami favorito, perícias e tipo de grau; conferir os nomes apresentados na ficha-base e na ficha de campanha sem JSON técnico.
...
- Testes e resultados: frontend — lint, suíte completa com 63 arquivos/376 testes e build aprovados em 2026-09-13. Quality Gate remoto com frontend e backend aprovados.
- Commit: `51ed559 feat(sessao): esclarece controles delegados`; documentação inicial `678a4fc`.
- Deploy/migration, se aplicável: sem migration ou seed; [Quality Gate 34736533336](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34736533336) aprovado e produção HTTP 200 confirmada.
...
- Commit: atualizações compatíveis registradas em `OPS-01`, `COD-01` e `COD-02`; `5402f4e` migra a configuração de seed para `prisma.config.ts`, sem alterar dependências.
- Deploy/migration, se aplicável: sem migration ou seed; [Quality Gate 34815870573](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34815870573) aprovado e produção HTTP 200 confirmada em 2026-09-14.
...
| 2026-09-14 | ARQ-01 | Utilitários puros de NPC (atributos, dados de perícia e normalização de tipos) foram separados da `SessaoService`, mantendo os adaptadores existentes. | `cbdff77`, `2d3c40f`; 2 cenários unitários, lint, build, `git diff --check`, [Quality Gate 34802432575](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34802432575) aprovado e produção HTTP 200. | Extrair montagem e privacidade de NPCs sem alterar autorização. |
| 2026-09-14 | ARQ-01 | Controle delegado, resumo público e filtro de NPCs ocultos passaram a usar o módulo puro de visão de NPC. | `c8b0bd6`, `0c1ef6d`; 3 cenários unitários, lint, build, `git diff --check`, [Quality Gate 34806326340](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34806326340) aprovado. | Extrair projeção persistente e filtros de eventos ocultos. |
...
| 2026-09-13 | UX-03 | Munição, tipo de grau e perícias extras de vinculados passaram a selecionar referências pelo catálogo, sem códigos manuais. | `7be42d5`, `41fbf3d`; o gate inicial detectou estado síncrono em efeito e a correção foi validada por frontend: lint, 379 testes, build e [Quality Gate 34767788939](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34767788939) aprovado. | Executar os casos manuais acumulados e revisar os painéis restantes. |
| 2026-09-13 | UX-03 | Habilidades próprias de técnica inata passaram a gerar código internamente, selecionar execução pelo catálogo e editar requisitos de habilidades e variações de forma estruturada. | `b9b9da0`; frontend: 379 testes e build; backend: lint, testes, build e `prisma validate`; [Quality Gate 34769737123](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34769737123) aprovado; produção HTTP 200. | Executar os casos manuais acumulados e revisar os painéis restantes. |
| 2026-09-13 | UX-03 | Tags de homebrew passaram de lista separada por vírgulas a chips editáveis; filtro técnico por código saiu do painel de técnicas. | `8504b4c`; frontend: 379 testes e build; [Quality Gate 34771728944](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34771728944) aprovado; produção HTTP 200. | Executar os casos manuais acumulados e revisar os painéis restantes. |
...
| 2026-09-13 | UX-03 | Livros e categorias passaram a selecionar cor por paleta visual, com alternativa nativa para cor personalizada. | `a7cf4e2`; frontend: 379 testes, build e `git diff --check`; [Quality Gate 34786691956](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34786691956) aprovado e produção HTTP 200. | Executar o caso manual acumulado e revisar os painéis restantes. |
| 2026-09-13 | UX-03 | Códigos do compêndio passaram a ser imutáveis nos contratos e no serviço, inclusive contra payload legado direto. | `d1df16c`, `6104b5b`; teste focado do backend com 15 casos, frontend: 379 testes/build aprovados localmente, [Quality Gate 34787927047](https://github.com/SenoPersonalProjects/assistenterpg-fullstack/actions/runs/34787927047) aprovado e produção HTTP 200. | Executar o caso manual acumulado e revisar os painéis restantes. |
